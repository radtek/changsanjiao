...
function TreeNode()
{
    ...
    this.Refersh = function () {
        ...
        //选择日期
        var fromDate = Ext.getDom("H00").value.toString();
        //alert(fromDate);
        var areaName = "";
        if (Ext.getDom("rd1").className == "radioChecked")
            areaName = "江苏";
        if (Ext.getDom("rd2").className == "radioChecked")
            areaName = "浙江";
        if (Ext.getDom("rd3").className == "radioChecked")
            areaName = "上海";
        if (Ext.getDom("rd4").className == "radioChecked")
            areaName = "安徽";

        if (Ext.getDom("会商PPT_0").className == "tabHighlight") {
            CurTab = "会商PPT";
            fileContainer = $("fileList");
        }
        if (Ext.getDom("预报简报_1").className == "tabHighlight") {
            CurTab = "预报简报";
            fileContainer = $("fileList1");
        }
        if (Ext.getDom("超级站_2").className == "tabHighlight") {
            CurTab = "超级站";
            fileContainer = $("fileList2");
        }
        ...
        currentNode.childrenNodes = null;
        self.path = "~/CJDATA/" + CurTab + "/" + AreaName + "/" + SELDATE + "/"
        currentNode.CreateChildren();
        fileContainer = $("fileList4");
        this.childrenNodes = null;
        currentNode.CreateChildrenII();
    }
   ...
   this.CreateChildrenII = function()
    {
        if (this.childrenNodes == null)
        {
            this.childrenNodes = new Array();
            var u = defaultURL + "?action=LISTII&value1=" + encodeURIComponent(self.path) + "&value2=";
        ...
        for (var i = 0; i < this.folders.length; i++)
        {
            fileContainer.appendChild(createFileView(this.folders[i].Name, true,this.folders[i].Size, this.folders[i].LastModify));
        }

        for (var i = 0; i < this.files.length; i++)
        {
            fileContainer.appendChild(createFileViewII(this.files[i].Name, false, this.files[i].Size, this.files[i].LastModify, this.files[i].creaters, this.files[i].createTime));
        }
    ...
    this.CreateChildren = function()
    {
        ...
        for (var i = 0; i < this.files.length; i++)
        {
            fileContainer.appendChild(createFileView(this.files[i].Name, false, this.files[i].Size, this.files[i].LastModify, this.files[i].creaters, this.files[i].createTime));
        }
...
var createFileView = function(fileName, isFolder, fileSize, modifyDate,creaters,createTime)
{
    ...
    var fileDates = createDiv();
    fileDates.className = "lastUpdate";
    fileDates.innerHTML = createTime;
    fileItem.appendChild(fileDates);

    var creater = createDiv();
    creater.className = "rename";
    creater.innerHTML = creaters;
    fileItem.appendChild(creater);
...
var createFileViewII = function (fileName, isFolder, fileSize, modifyDate, creaters, createTime) {
    var fileItem = createDiv("div");
    fileItem.className = "fileItem";
    ...
    fileNameSpan.onclick = function () {
        if (isFolder) {
            ...
            clickFileII(this.innerHTML, this.parentNode.parentNode.childNodes[2]);
        }
    }
