...
function changeDate(el){
    ...
    var week=weekday[startDate.getDay()];

    titleDate.innerHTML = forecastDate+" "+week;
    ...
    Ext.Ajax.request({ 
       ...
       params: {flag:"0",hour: forecastDate, period: period,Module:Module,tag:tag,moduleStyle:moduleStyle},
            success: function(response){
...
//            obj.className="save defaultButton";
 ...
 function createSummary(){
    ...
    var el=Ext.getDom("H00");
    var forecastDate = el.value;
    var  textValue = Ext.getDom("PH09");
    if(catualDate>forecastDate)
    {
            Ext.Ajax.request({ 
            url: getUrl('MMShareBLL.DAL.ComForecast','getContentEdits'),
            params: { forecastDate: forecastDate}, 
            success: function(response){
                if(response.responseText!=""){      
                      textValue.value=response.responseText;
                          textChange(textValue);
                    }
                    else 
                    {
                        textValue.value="【上海市空气质量预报】" + Ext.getDom("H11").value + hValue + "上海市环境监测中心 上海中心气象台";
                        textChange(textValue);
                    }
            }, 
            failure: function(response) { 
