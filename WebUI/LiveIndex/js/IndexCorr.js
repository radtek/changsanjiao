...
function displayRContent() {
    ...
    var rightWidth = ScreenWidth*0.2+"px";
    var leftWidth = ScreenWidth * 0.75 + "px";
    var fixed = 320;
    if (parseFloat(rightWidth) > fixed) {    //屏幕宽度的百分之二十超过320，则右侧的宽度固定显示320
        rightWidth = fixed+"px";
        leftWidth = ScreenWidth - fixed - 80 + "px";
    }
...
//单个指数重新计算
function one_Cal(e) {
    if (e.target.id == "oneCal") {
        e.stopPropagation();
    }
    var lst = $("#fore-time").text();
    var siteId = $("#site").val();
    var indexName = $("#oneCal").data().data()[2];
    var indexCode = $("#oneCal").data().data()[1];
    var index = $("#oneCal").data().selector.rows[0].rowIndex;
    Ext.Ajax.request({
        url: getUrl('MMShareBLL.DAL.LiveIndex', 'One_Cal'),
        params: { lst: lst, siteId: siteId, indexName: indexName, indexCode: indexCode },
        success: function (response) {
            var json = response.responseText;
            if (json.split("#")[0] == "ok") {
                $("#oneCal").css("display", "none");
                var data = eval('(' + json.split("#")[1] + ')').data[0];
                var val = data[3];
                var jb = data[4] + "级";
                var mean = data[5];
                var shortTip = data[6];
                ...
                for (i = 0; i < className.length; i++) {
                    $(".cal ." + className[i]).text(eval('(' + v[i] + ')'));
                }
                changeFontColor(indexCode);
                //if (indexCode.indexOf("rtssd") >= 0) {
                //    bodyIndex();    //取人体指数，如果计算的是人体指数则取预报，另一个去的是保存的数据
                //}
            } else {
                alert(data);
            }
...
function changeFontColor(id) {
    var indexVal = $("#"+id).parent("tr").children(".indexVal");
    var levelVal = $("#"+id).parent("tr").children(".levelVal");
    var val = [$(indexVal).text(), $(levelVal).text()];
    var invalid = [invalidValue, invalidLevel];
    var id = [indexVal, levelVal];
...
//改变详细信息的宽度
function changeWidth(gap) {    //gap第一次加载的时候右边滚动条是最后渲染的，要提前减去滚动条的宽度
    var total = $("#left-content").width();
    $("#J_detail").width(total-gap + "px");
    $("#J_jb").width(total - 390 + "px");   //390是原因、值得固定宽度的总和
    $("#_jb").width(total - 390 - 65 + "px");
    $("#J_tip").width(total - 140 + "px");
}
function getTable(id, obj) {
    //设置一个定时器，双击事件的时候不要触发单击事件
    var timer=null;
    hideRow();
    //右边要素信息表格清空
    ...
    var oTable = $('#' + id).DataTable({
        ...
        "columns": [
            ...
            //{ "visible": false },
            //{ "title": "指数名称", class: "mid indexName t" },
            //{ "title": "指数值", class: "short indexVal" },
            //{ "title": "指数级别", class: "short levelVal" },
            //{ "title": "含义", class: "mid indexMean t" },
            //{ "title": "简短提示", class: "l shortTip t" },
            //{ "title": "详细提示", class: "long longTip t" },
            //{ "title": "原因", class: "invisible" }
        ],
        "aoColumnDefs": [{
            //设置第一列不排序
    ...
    //组织浏览器的右击事件
    $('#' + id + ' tbody').bind("contextmenu", function () {
        return false;
    });
    $('#' + id + ' tbody').on('mousedown', 'tr', function (e) {
        if (3 == e.which) {
            //右击事件
            var x = e.clientX;
            var y = e.clientY;
            ...
            $(".cal").removeClass("cal");
            $(tr).addClass("cal");
           // $(window).on("scroll",changePositionOneCal);
            console.log(x,y);
        }
    });
    ...
    $('#' + id + ' tbody').on('click', 'tr', function () {
        ...
        var row = oTable.row(tr);
        clearTimeout(timer);
        timer=setTimeout(function(){getFeature(row.data()[2]);},300);
...
function getDropTable(e) {
    ...
    if($("#J_drop-down").css('display')=="block"){
        $("#J_drop-down").hide();
        return;
    }
    ...
    getTabGradeData();
    //下拉级别表格添加一个滚动事件
    $("#J_drop-down tr").eq(0).css("top", 0);
    var firstTrTop = $("#J_drop-down tr").eq(0).offset().top;
    $("#J_drop-down").on("scroll", function () {
        $("#J_drop-down tr").eq(0).offset({ "top": firstTrTop });
    })
}
...
function getTabGradeData() {
    //获取点击行的code
    $("#J_drop-down .drop-table tbody tr:eq(0)").siblings().remove();
    ...
    Ext.Ajax.request({
        ...
        success: function (response) {
            ...
            $("#J_drop-down").on("mouseenter", function () { mouseleaveDropTab("enter"); });
            $("#J_drop-down").on("mouseleave", mouseleaveDropTab);
            $(document.body).css({
                "overflow-y": "auto"
            });
        }
...
//鼠标移动到div上隐藏滚动条，并设置相应div的宽度否则会出现闪动，移开div滚动条显示
function mouseleaveDropTab(type) {
    if (type == "enter") {
        $(document.body).css({
            "overflow-y": "hidden"
        });
        var leftW = $("#left-content").width();   //左边内容的宽度
        var tab_headerW = $(".tab-header").width();   //表格头部的宽度
        var topW = $(".top").width();   //页面头部
        var topmarLeft = $(".top").css("margin-left");  //页面头部
        $(document.body).css({
            "overflow-y": "hidden"
        });
        $("#left-content").width(leftW);
        $(".top").css("margin-left", topmarLeft);
        $(".top").width(topW);
        $(".tab-header").width(tab_headerW);
        $("#_page").width(_pageW);
    } else {
        $(document.body).css({
            "overflow-y": "auto"
                ...
                for (var j = 0; j < temp.length; j++) {
                    ...
                    if (num > min && num < max && mark == 0) {
                        if (temp.length < value.length) {   //增加了某一个选项
                            ...
                            tip = tip.substring(0, wz_tip) + "且" + value[value.length - 1] + tip.substring(wz_tip, tip.length);
                            temp.push(str + "");
                        }
...
function changeColor(obj, flag) {   //1表示鼠标移上去事件，2表示鼠标移开事件，3表示点击事件
    if (flag == 1 && $(obj).css('background-color') != 'rgb(18, 85, 234)') {
        $(obj).css("background-color", "rgb(18, 85, 234)");
    } else if (flag == 2 && $(obj).css('background-color') == 'rgb(18, 85, 234)') {
...
//提示去掉没有选择的原因
function cancelInvalidRea(txt) {
    if (txt.indexOf('原因') > -1) {
        var startIndex = txt.indexOf('{');
        var endIndex = txt.indexOf('}');
        var temp = txt.substring(startIndex, endIndex + 1);
        txt = txt.replace(temp, '');
        return cancelInvalidRea(txt);
    } else {
        return txt;
    }
}
...
function save() {
    ...
    briefTip = cancelInvalidRea(briefTip);
    tip = cancelInvalidRea(tip);
    var code = $(".shown .details-control").attr('id');
            ...
            buttons: [{
                ...
                handler: function () {//点击时触发的事件
                    ...
                    Ext.Ajax.request({
                        ...
                        params: { type:"message",txt:messTxt,num:"1",name:"短信",operater:useName},
                        success: function (response) {
                            var data = response.responseText;
                            if(data=="success"){
                                alert("发布成功！");
...
function centerPre(){
    ...
    if(!pan){
        ...
        items: [
                {
                    id: "sevenPre",
                    title: "今日预报",  //这里两个标题写反了，为了方便只更新了标题，id的名称没有更改
