            ...
            init: function () {
                ...
                this.$http.post('WeChat.aspx/GetData').then(function (response) {
                    ...
                    if (days == 0) {       //获取的日期与当前的时间做对比，差值如果是0则是今天和明天，否则。。。
                        that.day = { "one": "今天", "two": "明天" };

                    } else {
                        that.day = { "one": "明天", "two": "后天" };
                    }
                }, function (response) {
                    //alert("获取数据失败");
                });
