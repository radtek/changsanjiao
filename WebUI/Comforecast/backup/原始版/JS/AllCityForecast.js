...
function DateChange(el) {
    ...
    for (var i = 0; i < aryDiv.length; i++) {
        if (aryDiv[i].id.substr(0, 3) == "Ptd") {
            var day = aryDiv[i].id.substr(3, 1)
    ...
    Ext.Ajax.request({
        url: getUrl('MMShareBLL.DAL.AllCityForecast', 'GetForecast'),
...
function HistoryForecast() {
    ...
    Ext.Ajax.request({
        url: getUrl('MMShareBLL.DAL.AllCityForecast', 'GetForecastHistory'),
        success: function (response) {
            clearElement(); //先清空所有数据
            if (response.responseText != "") {
                var result = Ext.util.JSON.decode(response.responseText);
...
function PublishForecast() {
    var postJson = "";
    var siteID = ["50745", "50873", "54453", "54337", "54324", "54237", "54497", "54347", "54094", "50850", "50853", "54827"];
    var itenID = [1, 2, 6, 5, 7, 3];
    var forecastDate = Ext.getDom("H00").value;
    for (var i = 0; i < siteID.length; i++) {
        for (var j = 1; j < 4; j++) {
            postJson = postJson + "H" + siteID[i] + j.toString() + ":";
            for (var k = 0; k < itenID.length; k++) {
                var id = "H" + siteID[i] + j.toString() + itenID[k].toString();
    ...
    postJson = postJson.substr(0, postJson.length - 1); //把表单信息和预报内容分隔开
    Ext.Ajax.request({
        url: getUrl('MMShareBLL.DAL.AllCityForecast', 'PublishForecast'),
...
function SaveForecast() {
    ...
    postJson = postJson.substr(0, postJson.length - 1); //把表单信息和预报内容分隔开
    Ext.Ajax.request({
        url: getUrl('MMShareBLL.DAL.AllCityForecast', 'SaveEdits'),
        params: { postJson: postJson, forecastDate: forecastDate },
        success: function (response) {
            if (response.responseText != "") {
                Ext.Msg.alert("信息", response.responseText);
            ...
            blur: function () {
                ...
                if (this.getValue() != divTag) {
                    ...
                    if (this.getValue() === "") {
                        ...
                        Ext.Ajax.request({
                            url: getUrl('MMShareBLL.DAL.AllCityForecast', 'AQIDescribe'),
...
function clearElement() {
    ...
    var aryDiv = comforecastTable.getElementsByTagName("span");
    for (var i = 0; i < aryDiv.length; i++) {
        if (aryDiv[i].id.substr(0, 1) == "H" && aryDiv[i].id.indexOf("primeplu") > 0) {
            aryDiv[i].innerHTML = "";
        }
    }   
}
