...
Ext.onReady(function () {
    ...
    //默认进入的时候点击查询按钮所做的查询
    doQueryChartAQI("朝阳市",olcCityCode);
   }
)
...
function doQueryChartAQI(citys,siteID) {
    ...
    myMask.show();
    Ext.Ajax.request({
        url: getUrl('MMShareBLL.DAL.TimePollutansForecast', 'GetAQIChartDBForecastZQL'),
                ...
                var i=0;
                    if(sps[i]!=""){
                      var result = Ext.util.JSON.decode(sps[i]);
                       RenderChartAQI(result, i, "#container"+(i+1)+""); 
                    }
                    i++;
            }
            else
    ...
    if (result != null) {
        ...
        $(contonter).highcharts({
            ...
            tooltip: {
                ...
                pointFormat:'<div style="color:{series.color}">  {series.name}准确率:  {point.y}%</div>',
                useHTML: true
            },
...
function clickQuery(){
       doQueryChartAQI(oldCityName,olcCityCode);
}
