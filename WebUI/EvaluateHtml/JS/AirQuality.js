...
function addSite(sender, id) {
    ...
    var liHtml = String.format("<span id='T{0}'><a href=\"javascript:tabClick('T{0}');\">{1}<img src='css/image/b_close.png' class='close_ico' onclick=\"closeTab('T{0}');\" /></a></span>", id, name);
    li.innerHTML = liHtml;
...
function tabClick(id) {
    ...
    if (lastEl != null) {
        ...
        //    a.href = String.format("javascript:tabClick('{0}');",lastTabID);
        //    a.innerHTML = lastEl.innerHTML;
        //    lastEl.innerHTML = "";
    ...
    //根据切换的污染物，改变显示的数据
    //    var aryDiv=Ext.query("div","siteDataTable");
    //    for(var i=0;i<aryDiv.length;i++){
    //        var divID = aryDiv[i].id;
    //        if (divID.substr(0,1) == "H"){
    //            if(divID.substr(divID.length-3,3) == id.substr(1,id.length-1))
    //                showEl(divID);
    //            else  
    //                hideEl(divID);
    //        }
...
function querySiteData() {
    ...
    myMask.show();
    Ext.Ajax.request({
        url: getUrl('MMShareBLL.DAL.EvalutionCaculate', 'Query'),
        params: { fromDate: fromDate, toDate: toDate, siteIDs: siteIDs, curSiteID: curSiteID },
...
function exportSiteData() {
    ...
    var toDate = Ext.getDom("H01").value;
    var curSiteID = lastTab.substr(1, lastTab.length - 1);
    var content = fromDate + "|" + toDate + "|" + curSiteID;
