...
function trickData(year) {
    Ext.Ajax.request({
        url: getUrl('MMShareBLL.DAL.ScoreData', 'retrunChinaYear'),
        params: { year: year },
...
function chinaYearClick(time, userName, el) {
    var obj = el;
    var f1 = $(obj).children('td').eq(2).html();
    var f2 = $(obj).children('td').eq(3).html();
    var f3 = $(obj).children('td').eq(4).html();
    var totoalScore = $(obj).children('td').eq(5).html();
    Ext.Ajax.request({
        url: getUrl('MMShareBLL.DAL.ScoreData', 'retrunChinaYearList'),
        params: { time: time, userName: userName, f1: f1, f2: f2, f3: f3, totoalScore: totoalScore },
        success: function (response) {
            if (response.responseText != "") {
