<%@ Page Language="C#" AutoEventWireup="true" CodeFile="WriteData.aspx.cs" Inherits="EvaluateHtml_WriteData" %>
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    <title></title>
    <link href="../DatePicker1/skin/WdatePicker.css" rel="stylesheet" />
    <link href="css/WriteData.css" rel="stylesheet" />

 <%--   <script src="../DatePicker1/WdatePicker.js"></script>--%>
                ...
                <div class="period-content">
                    <table style="width:100%;border-collapse:collapse;">
                        <thead>
                            <tr>
                                <td></td>
                                <td v-for="(item,key,index) in periodData.data.上午" v-html="item.poll" ></td>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(item,key,index) in periodData.data">
                                <td>{{key}}</td>
                                <td v-for="(val,index) in item">
                                   <%-- <input  v-model="item[key]" :readonly="(!item[key])!=true" @blur="setValue(item,key)"/>--%>
                                     <input v-model="val.val" :readonly="his?true:((val.isEdit=='0')?true:false)" @focus="focus($event,val.isEdit)" @blur="blur($event)" />
                                </td>
                            </tr>
                        </tbody>
                    </table>
