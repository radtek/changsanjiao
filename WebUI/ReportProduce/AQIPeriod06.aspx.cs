...
public partial class ReportProduce_AQIPeriod48 : System.Web.UI.Page
{
    ...
    /// <returns></returns>
    private void CreateTableCopy(DateTime dtNow, int backDays)
    {
    ...
    private void CreateTable(DateTime dtNow, int backDays)
    {
        ...
        for (int i = 0; i <= 27; i++)
        {
            ...
            int t = 0;//属于那一天 3,4,5
            int k = 0;//时段， 上半夜（20-0）4，下半夜（0-6）1，上午（6-12）2，下午（12-28）3，夜间（20-6）6，日平均 7
            string strprefix = "";   //后来增加的，用来表示第三日的两家综合的还是单独的      //wb  2017.6.1
            //if (i == 8 || i == 18) tableClass = "tablerow4Out";
            //else
            //    tableClass = "tablerowOut";
            tableClass = "tablerowOut";
            ...
            else if (i == 20)
            {
                td.Attributes.Add("rowspan", "4");
                td.Attributes.Add("class", "tablerow4Out");
                td.InnerHtml = string.Format("72小时");
                newRow.Cells.Add(td);
                td = new HtmlTableCell();
            }
            else if (i == 24)
            ...
            if (i == 22)
            {
                td.Attributes.Add("class", "tablerowOutP");
                td.Attributes.Add("rowspan", "2");
                td.InnerHtml = string.Format("<span id ='Ptd{0}1'></span>", 8);
                newRow.Cells.Add(td);
                td = new HtmlTableCell();
            ...
            if (i == 26)
            {
                td.Attributes.Add("class", "tablerowOutP");
                td.Attributes.Add("rowspan", "2");
                td.InnerHtml = string.Format("<span id ='Ptd{0}1'></span>", 7);
                newRow.Cells.Add(td);
                td = new HtmlTableCell();
            }
            ...
            //else
            //    tableClass = "tablerowOut";
            //如果是第三天全天则合并所有列，即只需要一列   //wb  2017.6.1s
            if (i == 22 || i==23)
            {
                string str = "";  //存储第三日全天气象局的标签内容
                string classDiv = "txt";
                //id号的长度增加了一个以区分污染物浓度与首要污染物，v表示污染物浓度，p表示首要污染物
                str=sb.Append(string.Format("<label for='{0}{1}{2}{3}{4}{6}'>AQI:</label><input type='text' id='{0}{1}{2}{3}{4}{6}'class='{5}'<label for='{0}{1}{2}{3}{4}{7}'>首要污染物:</label><select id='{0}{1}{2}{3}{4}{7}'class='{5}'></select>", strprefix, t, "2", k, '0', classDiv,"v","p")).ToString();
                sb.Length = 0;
                sb.Append(string.Format("<label for='{0}{1}{2}{3}{4}{6}'>AQI:</label><input type='text' id='{0}{1}{2}{3}{4}{6}'class='{5}'<label for='{0}{1}{2}{3}{4}{7}'>首要污染物:</label><select id='{0}{1}{2}{3}{4}{7}'class='{5}'></select>", strprefix, t, "1", k, '0', classDiv,"v","p"));
                tableClass = "tablerowOut special";
                td = new HtmlTableCell();
                td.Attributes.Add("class", tableClass);
                td.Attributes.Add("colspan", "12");
                if (i == 22)
                {
                    td.InnerHtml = sb.ToString() + str;
                }
                else { 
                    td.InnerHtml = sb.ToString(); 
                }
                ...
                for (int j = 1; j <= 6; j++)
                {
                    if (j == 6)
                    {
                        if (i == 13 || i == 3)
                            ...
                            tableClass = "tablerowRightTopColorOut";     //与tablerowRightColorOut、rowColorOut、rowOut、tablerowRightBottomColorOut样式一样
                        else if (i == 6 || i == 16 || i == 5 || i == 7 || i == 8 || i == 16 || i == 15 || i == 17 || i == 18)
                            tableClass = "tablerowRightColorOut";
                        else if (i == 9 || i == 19)
                            tableClass = "tablerowRightBottomColorOut";
                    ...
                    if (DateTime.Now > DateTime.Parse(DateTime.
                             Now.ToShortDateString() + " 20:00:00")) {
                                 onClick = "onclick = 'showInput(event,this)'";//薛辉
                    }
                    td.Attributes.Add("class", tableClass);
                    string classDiv = "show";
                    //判断是否已经到20时 薛辉
                    if (DateTime.Now > DateTime.Parse(DateTime.
                        Now.ToShortDateString() + " 20:00:00")) {
                            classDiv = "divInputType show";//新需求
                    }
                    td.Attributes.Add("colspan", "2");
                    if (j == 6)
                        sb.Append(string.Format("<div id = 'H{0}{3}{1}{2}' class = '{4}' {5}>/</div>", t, k, j, "1", classDiv, onClick));//(j+1)%2环境监测中心0，气象局1
