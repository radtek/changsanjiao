...
function getWindBarbData(windSpeed, windDir) {
    var windObj = {};
    var windBarb = new Array();
    var speed = "";
    var direct = "";
    var time = new Array();
    //风向和风速的时间是对应的，这里只循环一个就可以了
    for (var i = 0; i < windSpeed.length; i++) {
        var temp = new Array();
        speed = windSpeed[i][1];
        direct = windDir[i][1];
        time.push(windDir[i][0]);
        temp.push(speed, direct);
        windBarb.push(temp);
    }
    windObj.windBarb = windBarb;
    windObj.windX = time;
    return windObj;
}
function RenderChart(data) {
    ...
    if (windArr.length > 1) {
        ...
        windInfoObj = getWindBarbData(windSpeed, windDir);
