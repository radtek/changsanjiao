...
public partial class ExtentionPeriod_GroundEle : System.Web.UI.Page
{
    ...
    public static string GetTime() {
        string sql = "SELECT TOP 1 startTime FROM dbo.T_CFSEleSH ORDER BY endTime DESC,period DESC ";
    ...
    public static string Query(string time) {
        string txt = "";
        string sql = "SELECT Ele,startTime,endTime,period,value FROM dbo.T_CFSEleSH WHERE startTime='"+time+"' ORDER BY endTime ASC";
        int interval = 6,periods = 0;   //时效间隔，开始的时效
        try
        {
            DataTable dt = m_Database.GetDataTable(sql);
    ...
    public static List<string> GetEle() {
        ...
        string sql = "SELECT DISTINCT(ele) FROM dbo.T_CFSEleSH ORDER BY Ele ASC";
        DataTable dt = m_Database.GetDataTable(sql);
        if (dt != null && dt.Rows.Count > 0) {
