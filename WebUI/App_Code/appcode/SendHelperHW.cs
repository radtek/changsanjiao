...
public class SendHelperHW
{
    ...
    public static string GetHtml(string title, List<string[]> infoes)
    {
        ...
        //string imgUrl = "http://10.228.177.62:8282/CancelRequest/email/";内网地址
        string typeURL = "";
        string visibility = "";
        if (infoes[0][0].IndexOf("哮喘") >= 0)
        {
            typeURL = "etxc.png";
            visibility = "display:block;margin-top:10px;";
        }
        ...
        html2 += "<img width='141' height='110' src='" + (imgUrl + typeURL) + "'/><br><span style='color:black;' align='center'>" + GetPeople(infoes[0][0]) + "</span>";
        ...
        string html = "<div class='littletitle' style='" + visibility + "'><h4 class='headtext'>" + title + "</h4></div>";
        if (infoes[0][0].IndexOf("儿童感冒") >= 0 || infoes[0][0].IndexOf("哮喘") >= 0 || infoes[0][0].IndexOf("COPD") >= 0 || infoes[0][0].IndexOf("重污染") >= 0)
        {
            html += "<div class='radius'><table class='uptable'cellspacing='0' cellpadding='5' border='0' bgcolor='#e9e9e9' width='100%'>";
        }
        html += "<tr   bgcolor='#F0F5FB' style='color:#000'><td class='imgtd'>" + html2 + "</td>";

        if (infoes[0][2].Contains("中暑"))
        {
            html = "<h4 class='headtext'>" + title + "</h4><table class='uptable' cellspacing='1' cellpadding='5' border='0' bgcolor='#e9e9e9' width='100%'>"
            + "<tr bgcolor='#1458d7' style='color:#fff'>";
        }

        //for (int i = 0; i < infoes.Count; i++)
        //{
        //    html += "<td style='font-size:16px; font-family:微软雅黑; width:40%; color:black '><strong>" + infoes[i][1] + "</strong></td>";
        //}
        ...
        for (int i = 0; i < infoes.Count; i++)
        {
            if (infoes[0][2].Contains("中暑"))
            {
                html += "<td class='uptd'><div class='space'></div><strong>" + infoes[i][1] + "</strong><br/><br/><strong>风险等级：</strong><img src='" + imgUrl + type + GetLvl(infoes[i][2]) + "'><br/><br/><strong>防护建议：</strong>" + infoes[i][4] + "</td>";
            }
            else
            {
                html += "<td class='uptd'><div class='space'></div><strong style='font-size:16px'>" + infoes[i][1] + "</strong><br/><div class='space'></div>风险等级：   <img src='" + imgUrl + type + GetLvl(infoes[i][2]) + "'><br/><div class='space'></div>防范人群：" + infoes[i][3] + "<br/><div class='space'></div>防护建议：" + infoes[i][4] + "<div class='space'></div></td>";
            }
        }
        if (infoes[0][0].IndexOf("老年人感冒") >= 0 || infoes[0][0].IndexOf("哮喘") >= 0 || infoes[0][0].IndexOf("COPD") >= 0 || infoes[0][0].IndexOf("重污染") >= 0)
        {
            html += "</tr></table></div>";
        }
        return html;
