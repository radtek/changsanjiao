...
public class SendHelper
{
    ...
    public static string GetHtml(string title, List<string[]> infoes)
    {
        ...
        else if (infoes[0][0].IndexOf("青年感冒") >= 0)
        {
            typeURL = "qsn.png";
        }
        else if (infoes[0][0].IndexOf("老年感冒") >= 0)
        {
            typeURL = "lnr.png";
        }
        ...
        html2 += "<img width='101' height='70' src='" + (imgUrl + typeURL) + "'/><br><span style='color:black'>" + GetPeople(infoes[0][0]) + "</span>";

        string html = "<h4 style='margin:15px 0 8px 0;;font-weight: bold;'>" + title + "</h4><table cellspacing='1' cellpadding='5' border='0' bgcolor='#e9e9e9'>"
            + "<tr   bgcolor='#fafafa' style='color:#fff'><td rowspan='2' style='font-size:16px; font-family:微软雅黑; width:10%; text-align:center;'>" + html2 + "</td>";

        if (infoes[0][2].Contains("中暑"))
        {
            html = "<h4 style='margin:15px 0 8px 0;font-weight: bold;'>" + title + "</h4><table cellspacing='1' cellpadding='5' border='0' bgcolor='#e9e9e9'>"
            + "<tr bgcolor='#1458d7' style='color:#fff'>";
        }

        for (int i = 0; i < infoes.Count; i++)
        {
            html += "<td style='font-size:16px; font-family:微软雅黑; width:35%; background-color:#1458d7 '><strong>" + infoes[i][1] + "</strong></td>";
        }
    ...
    public static string GetTitle(string day, string region, string healthyType,string siteName)
    {
       // return region + "气象台" + day + "发布" + healthyType + "气象风险预报（试行）";
        ...
        switch (healthyType) {
            ...
            case "COPD": ht = "COPD"; return region + "" + day + "发布" + (siteName == "中心城区" ? "上海市" : siteName) + "" + ht + "气象环境风险预报";
            case "儿童哮喘": ht = "儿童哮喘"; return region + "" + day + "发布" + (siteName == "中心城区" ? "上海市" : siteName) + "" + ht + "气象环境风险预报";
            default: ht = healthyType; break;
        }

        return region + "" + day + "发布" + (siteName=="中心城区"?"上海市":siteName) + "" + ht + "气象风险预报";
    }
    ...
    private static string GetPeople(string type)
    {
        switch (type)
        {
            case "儿童感冒": return "儿童感冒";
            case "青年感冒": return "青少年和成年人感冒";
            case "老年感冒": return "老年人感冒";
            case "儿童哮喘": return "儿童哮喘";
            case "重污染": return "--";
