<%@ Page Language="C#" AutoEventWireup="true" CodeFile="FreeList.aspx.cs" Inherits="ImgList_FreeList" %>
...
<head runat="server">
    ...
    <script language="javascript" type="text/javascript" src="JS/FreeList.js"></script>
...
<body id="Body1" runat="server" style="-webkit-overflow-scrolling:touch; overflow: auto;" >
    ...
    <div class="ListContent" id="ListContent">
        ...
        <div  class="dateSelect" id="areaSelect">
        <label  class="cur-select"></label>
        <select id="area" >
        <option value="01" selected="selected">全国</option>
        <option value="02">华东</option>
        <option value="04">长三角</option>
        <option value="03">上海</option>
        </select>
        </div>
        <div id="layers" class="dateSelect" style=" margin-top:40px;">
        </div>
        <div id="types" class="dateSelect" style=" margin-top:80px;">
        </div>
        <div class="label" style="margin-bottom: 2px;">时间</div>
        <div class="date">
        <input name="" type="text" id="time" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" onchange="changeDate(this)" class="formstyle" runat="server"/>
        <div  class="hourSelect" id="hour" style=" float:right;"></div>
        </div>
        <div  id="sure" class="buttonDiv"><input  value="确定" class="addBut" type="button" onclick="addImgButton()"/></div>
        </div>
    </div>
</body>
</html>
