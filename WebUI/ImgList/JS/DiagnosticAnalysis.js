...
function cityChange(city) {
    if (city != "" && city != undefined) {
        var layerTableName = $('#area option:selected')[0].value;
        Ext.Ajax.request({
            url: getUrl('MMShareBLL.DAL.FreeQuery', 'cityChange'),
            params: { layerTableName: layerTableName, city: city },
...
function trickLayers(stationId) {
    nowType = stationId;
    Ext.Ajax.request({
        url: getUrl('MMShareBLL.DAL.FreeQuery', 'trickDiagnosticLayers'),
        params: { layerTableName: stationId },
        success: function (response) {
            if (response.responseText != "") {
                var result = Ext.util.JSON.decode(response.responseText);
                changeDateSucessed(result);
...
function addImgButton() {
    if (oldArr.length > 0) {
        ...
        Ext.Ajax.request({
            ...
            params: { forecastDate: forecastDate, types: types, layers: layers, time: time,city: city },
            success: function (response) {
                if (response.responseText != "") {
