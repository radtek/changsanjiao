    ...
    $.ajax({
      ...
      success: function (responseData) {
        ...
        if (data != null) {
          ...
          alert("站点(" + StationID + ") 在时间段:" + startTime + "至" + endTime + " 无能见度等级数据");
        }
 ...
 * @param {arr} seriesArr 
 * @param {arr} xAxisVal 
 * @param {arr} timeMMDDArr 
 */
//根据请求到的数据生成折线图
function createChart(seriesArr, xAxisVal, timeMMDDArr) {
  drawChart = Highcharts.chart('chartContainer', {
    chart: {
      type: 'line',
      height: 300,
    ...
    tooltip: {
      ...
      pointFormatter: function () {
        //console.log(this.index);
        return '<p>' + timeMMDDArr[this.index] + '</p> <p>' +
  ...
  $.each(seriesArr, (i, item) => {
    drawChart.addSeries(item);
  })
    ...
    $.ajax({
      ...
      success: function (responseData) {
        ...
        if (data != null) {
          ...
          alert("站点(" + StationID + ") 在时间段:" + startTime + "至" + lstTime + " 无能见度值数据");
        }
        //就算没数据，也要显示图例
        //向series中添加数据
        var seriesArr = [{
    ...
    $.ajax({
      ...
      success: function (responseData) {
        ...
        if (data != null) {
          ...
          alert("站点(" + StationID + ") 在时间段:" + startTime + "至" + lstTime + " 无lightgbm数据");
        }
        //没数据也要显示图例
        var seriesArr = {
          name: 'lightgbm',
    ...
    $.ajax({
      ...
      success: function (responseData) {
        ...
        if (data != null) {
          ...
          alert("站点(" + StationID + ") 在时间段:" + startTime + "至" + lstTime + " 无能见度实况数据！");
        }
        //没数据也要显示图例
        var seriesArr = {
          name: '能见度实况值',
