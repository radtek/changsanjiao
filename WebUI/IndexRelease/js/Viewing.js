...
function publishWB() {
    ...
    Ext.Ajax.request({
        url: getUrl('MMShareBLL.DAL.IndexRelease', 'PublishWB'),
        params: { period: period, type: type },
        success: function (response) {
            if (response.responseText == "OK") {
                alert("发布成功！");
                ...
                alert("图片上传失败,错误信息为：" + response.responseText)
            }
        }
    });
}
...
function DownloadWB() {
    ...
    Ext.Ajax.request({
        url: getUrl('MMShareBLL.DAL.IndexRelease', 'DownloadWB'),
        params: { period: period, type: type },
        success: function (response) {
            if (response.responseText.toString().indexOf("wb.png") >= 0 || response.responseText.toString().indexOf("wb.jpg") >= 0) {
