...
ImageFrame = function (fieldInfo, entity) {
    ...
    function queryList(listID, isInitial) {
        ...
        var store = new Ext.data.JsonStore({
            url: getUrl('MMShareBLL.DAL.Forecast', 'QueryTextList'),
    ...
    function selectChange(view, nodes) {
        ...
        Ext.Ajax.request({
            url: getUrl('MMShareBLL.DAL.Forecast', 'trickText'),
            params: { Datetime: triggerValue, entityName: entityName },
            success: function (response) {
                if (response.responseText != "") {
                    var result = response.responseText;
                    var title = Ext.getDom("title");
                    var author = Ext.getDom("author");
                    var writing = Ext.getDom("writing");
                    var array = result.split('*');
                    title.innerHTML = array[0];
                    author.innerHTML = array[1];
                    writing.innerHTML = array[2];
