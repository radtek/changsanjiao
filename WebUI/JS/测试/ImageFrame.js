...
ImageFrame = function (fieldInfo, entity) {
    ...
    if (fieldInfo.length > 0)//new code
        DateNames = fieldInfo[0].Name;
    ...
    fieldControl = new Ext.ListView({
        ...
        listeners: {
            ...
            } //refresh  prepareData bufferRender  bindStore  refreshNode
    ...
    var playTaskRefresh = {
        ...
        interval: 50000 //1小时3600000当页面一直放着不动的话，过1小时就会自动刷新
    }
    ...
    function AddKeyEvent(viewID) {
        ...
        new Ext.KeyNav(el, {
            ...
            up: function (e) {
                MoveSelection(component, -1);
                //var c = Ext.getCmp(viewID).store.data.items
                //alert(c.length);
            },
            right: function (e) {
                MoveSelection(component, 1);
    ...
    function MoveSelection(view, step) {
        ...
        if (selectionsArray.length > 0) {
            ...
            fieldControl.select(selectedIndex);
            //selectChange(view, null);
        }
    }
    ...
    function queryList(listID, isInitial) {
        ...
        var fieldControl = Ext.getCmp(listID); //得到查询结果    by xuehui
        fieldControls = Ext.getCmp(listID);
        var store = new Ext.data.JsonStore({
            ...
            listeners: { "load":
                            ...
                            if (selectDateTime != "") {
                                if (nodes != null && nodes.length > 0) {
                                    for (i = 0; i < nodes.length; i++) {
                                        if (nodes[i].innerText != null) {
                                            if (nodes[i].innerText == selectDateTime) {
                                                fieldControl.select(i);
                                                break;
                                            } else {
                                                fieldControl.select(i);
                                            }
                    ...
                    function () {
                        ...
                        addDate();
                        if (fieldControl.getNodes().length <= 0)
                            RefreshData();
...
Ext.extend(ImageFrame, Ext.Panel, {
    beforeDestroy: function () {
        ...
        var startDateCmp = Ext.getCmp(DateNames + 'startDate');
        var startHourCmp = Ext.getCmp('hour' + startDateCmp.getId());
        var startDate = startDateCmp.getValue().add(Date.HOUR, startHourCmp.getValue());
        selectStartTime = startDate.format('Y-m-d H:i:s');

        var endDateCmp = Ext.getCmp(DateNames + 'endDate');
        var endHourCmp = Ext.getCmp('hour' + endDateCmp.getId());
        var endDate = endDateCmp.getValue().add(Date.HOUR, endHourCmp.getValue());
        selectEndTime = endDate.format('Y-m-d H:i:s');

        if (fieldControls != null && fieldControls.getSelectedNodes().length > 0) {
            selectDateTime = fieldControls.getSelectedNodes()[0].innerText;
        }
    }
});
...
function RefreshData() {
    ...
    Ext.Ajax.request({
        url: getUrl('MMShareBLL.DAL.Forecast', 'GetEntity'),
        params: { entityName: entityNameValue },
        success: function (response) {
            var result = Ext.util.JSON.decode(response.responseText);
            fieldInfo = result;
            for (var i = 0; i < fieldInfo.length; i++) {
                ...
                if (fieldValue.FieldType == 8) {
                    ...
                    var startDateCmp = Ext.getCmp(fieldValue.Name + 'startDate');
                    var startHourCmp = Ext.getCmp('hour' + startDateCmp.getId());
                    if (entityValue.realtime == "" || entityValue.realtime.indexOf("R") == 0) {
                        var startDate = convertDate(lastTimes[0]);
                        var backDay = -1;
                        if (entityValue.realtime.indexOf("R") == 0)
                            backDay = -parseInt(entityValue.realtime.substr(2));
                        startDate = startDate.add(Date.DAY, backDay);
                        var hour = lastTimes[1].substring(0, 2);
                        startHourCmp.setValue(hour);
                        startDateCmp.setValue(startDate.format('Y-m-d'));
            ...
            isSuccess = true;
            dis(entityNameValue, false, entityValue);
        },
        failure: function (response) {
            //Ext.Msg.alert("错误", "请求失败，错误代码为：" + response.status);
        }
...
//点击查询以后罗列出查询列表
function dis(listID, isInitial, entityValue) {
    ...
    var store = new Ext.data.JsonStore({
        url: getUrl('MMShareBLL.DAL.Forecast', 'QueryList'),
        baseParams: { entityName: listID, entityObj: entityObj },
        autoLoad: true,
        fields: ['DM', 'MC'],
        listeners: { "load":
                    function () {
...
function addDate() {
    ...
    var startDateCmp = Ext.getCmp(DateNames + 'startDate');
    var startHourCmp = Ext.getCmp('hour' + startDateCmp.getId());
    var hour = selectStartTime.substring(13, 11);
    var date = selectStartTime.substring(0, 10);
    var year = date.substring(0, 4);
    var month = date.substring(7, 5);
    var day = date.substring(10, 8);
    var dateFromt = AddDays(year + "/" + month + "/" + day, 1);
    startHourCmp.setValue(hour);
    startDateCmp.setValue(dateFromt);

    var endDateCmp = Ext.getCmp(DateNames + 'endDate');
    var endHourCmp = Ext.getCmp('hour' + endDateCmp.getId());
    hour = selectEndTime.substring(13, 11);
    date = selectEndTime.substring(0, 10);
    var year = date.substring(0, 4);
    var month = date.substring(7, 5);
    var day = date.substring(10, 8);
    dateFromt = AddDays(year + "/" + month + "/" + day, 1);
    endHourCmp.setValue(hour);
    endDateCmp.setValue(dateFromt);
...
function initDate() {
    ...
    if (selectStartTime != null && selectStartTime != "") {
        var startDateCmp = Ext.getCmp(DateNames + 'startDate');
        var startHourCmp = Ext.getCmp('hour' + startDateCmp.getId());
        var hour = selectStartTime.substring(13, 11);
        var date = selectStartTime.substring(0, 10);
        startHourCmp.setValue(hour);
        startDateCmp.setValue(date);
