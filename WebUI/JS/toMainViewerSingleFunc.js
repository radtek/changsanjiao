...
function InitialViewer(loginTime, product) {
    ...
    function itemClick(action, iconCls) {//Guidance
        ...
        switch (action.id) {
            case "airQuality":
                addHtmlPanelSingleFunc("AirQualitySingle.aspx");
                break;
            case "jgRadar":
                addHtmlPanelSingleFunc("JgRadarSingle.aspx");
                break;
            case "guidance":
                addHtmlPanelSingleFunc("GuidanceSingle.aspx");
                break;
            case "diagnostic":
                addHtmlPanelSingleFunc("DiagnosticSingle.aspx");
                break;
//            case "airQuality": case "xsforcast": case "guidance": case "diagnostic": case "innp": case "jgRadar": case "comForecasts": case "SuperStation": case "WeatherPollution": case "systemManage": case "dayForecast":
            case "xsforcast": case "SuperStation": case "innp": case "comForecasts": case "WeatherPollution": case "systemManage": case "dayForecast":
...
//        else
    ...
    function addHtmlPanelSingleFunc(iframeSrc) {
        if (west.hidden == false)
            west.hide();
        ...
        var htmPanel = new Ext.Panel({
            ...
            header: false,
            layout: "fit",
            html: "<div id='frameID' style='width:100%;height:100%" + ";-webkit-overflow-scrolling:touch; overflow: auto;'><iframe id='gis' src='" + iframeSrc + "' height='100%' width='100%' frameborder='0'></iframe></div>"
        });
        center.removeAll();
        center.add(htmPanel);
        center.doLayout();
