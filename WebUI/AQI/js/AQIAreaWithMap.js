...
Ext.onReady(function () {
    ...
    $("#textPart").height(pageHeight - 479-43);
    $("#aqiText").height(pageHeight - 469 - 50-43);
            ...
            $(m).click(function () {
                ...
                if (n.id == "firstPolUl_58367") {
                    ...
                    Ext.Ajax.request({
                        ...
                        params: { firstItem: $(m).text().trim(), aqi: aqiValue },
                        success: function (response) {
                            if (response.responseText != "") {
        ...
        $(n).change(function () {
            ...
            if (currentInputName == "AQI") {
                ...
                if (newAQIValue == null || newAQIValue == "" || !isFloat(newAQIValue)) {
                    ...
                    if (n.id == "58367_AQI") {
                        //                    var firstItem = $($("#58367_Item").find("div")[0]).text();
                        var firstItem = $("#58367_Item").text().trim();
                        Ext.Ajax.request({
                            url: getUrl('MMShareBLL.DAL.AQIForecast', 'CalculateAQLLevelAndReplaceNew'),
...
function setTextValue() {
    var value = $("#aqi_o3").val();
    if (value == "") {
        return;
    }
    var reg = /^(([0-9]\.[0-9]*)|([1-9][0-9]*\.[0-9]+)|([1-9][0-9]*))$/;
    if (!reg.test(value)) {
        alert("请出入合法数字");
        return;
    }
    value = convertSixNum(value);
    var _text = $('#dataFileContent').val();
    var rowText = _text.split('\n');
    var finalText = "";
    for (var row = 0; row < rowText.length; row++) {
        if (row == 9) {
            let changeRowArr = rowText[9].split(' ');
            let val = "";
            for (let col = 0; col < changeRowArr.length; col++) {
                if (col == 5) {
                    val += value+" ";
                }else {
                    val += changeRowArr[col]+" ";
                }
            }
            finalText += val.trimRight(' ')+"\n";
        } else {
            finalText += rowText[row]+"\n";
        }
    }
    $('#dataFileContent').val(finalText.trimRight('\n'));
}
function convertSixNum(num) {
    //106.02    ==>010602
