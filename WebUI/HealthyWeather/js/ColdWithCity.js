...
var childValues = { '1': '无影响', '2': '体质非常差的小朋友', '3': '体质差的小朋友', '4': '体质或发育差的小朋友', '5': '所有小朋友', '6': '所有小朋友' };
var yougValuess = { '1': '无影响', '2': '体质非常差的青少年和成年人', '3': '体质差的青少年和成年人', '4': '体质差和亚健康的青少年和成年人', '5': '所有青少年和成年人', '6': '所有青少年和成年人' };
var oldValues = { '1': '无影响', '2': '体质非常差的老人', '3': '体质差的老人', '4': '体质差和亚健康的老人', '5': '所有老人', '6': '所有老人' };
...
Ext.onReady(function () {
  ...
  //  $("#forecastTime").val(t);
    ...
    //    var h1 = $("#c1").height();
    //    alert(h1);
    //    $("#c2").css({ marginTop: (h1 + 10) });
    //    $("#c3").css({ marginTop: ((h1 * 2) + 10) });
    ...
    $("#foreSave").click(function () {
        ...
        //历史数据不能保存
        var forecastTimeV = Ext.getDom("forecastTime").value;
        var serverTimeV = Ext.getDom("serverTime").value;
        if (forecastTimeV != serverTimeV) {
            alert("历史数据不能修改保存！");
            return;
        }
        ...
        var forecastTime = Ext.getDom("forecastTime").value;
        var type = "感冒";
        var priod;
        if (oldPeriod == "p10") {
            priod = "10";
            ...
            priod = "17";
            if (user != "管理员") {
                var isSave = canSave(user, type, forecastTime);
                if (isSave!="ok") {
                    alert("17时没有预报数据，请稍后再保存");
                    return;
                }
            }
        }
...
//如果是17时保存之前判断是否有预报的数据，如果没有则不能保存
function canSave(user, type, forecastTime) {
    var flag = "error";
    $.ajax({
        url: getUrl('MMShareBLL.DAL.HealthyWeather', 'CanSave'),
        type: "GET",
        contentType: "application/json;charset=utf-8",
        data: { userName:user,type:type,forecastDate:forecastTime},
        dataType: 'json',
        async:false,
    ...
    $.each($(".firstPolText"), function (i, n) {
        ...
        if (index == 1 || index == 2) {
            ...
            if (i > 49 && i <= 59) {
                ...
                z = eval(oldValues);
                var cx = $($(".aqiValue")[i]);
                cx.val(z[polLevelIndexs]);//薛辉  2017-02-15   动态改变等级选择后的防范人群信息
            }
        }
    ...
    Ext.Ajax.request({
        ...
        success: function (response) {
            if (response.responseText != "") {
                ...
                //设置最近数据更新时间 2017-03-13 xuehui
                var v = response.responseText.split('@')[1];
            ...
            if (flag == "1") {
                ...
                if (Ext.getDom("dd1er").className == "radioChecked")
                    RefershMap("1");
                else
                    RefershMap("2")
            }
...
function refershYougForecast(flag) {   //先获取预报数据，在获取保存的数据。即有保存的数据就显示保存数据，没有就显示预报的数据
    var user = Ext.getDom("forecaster").innerText;
    var forecastTime = Ext.getDom("forecastTime").value;
    var priod;
    if (oldPeriod == "p10")
    ...
    Ext.Ajax.request({
        ...
        success: function (response) {
            ...
            if (flag == "1") {  //flag为1 表示要获取保存的数据
                refershYoug();
            }
            else {
                if (Ext.getDom("dd3er").className == "radioChecked")
                    RefershMap("3");
                else
                    RefershMap("4")
            }
    ...
    Ext.Ajax.request({
        ...
        success: function (response) {
            ...
            if (flag == "1") {
                ...
                if (Ext.getDom("dd5er").className == "radioChecked")
                    RefershMap("5");
                else
                    RefershMap("6")
            }
...
function radioClickModule(id) {
    ...
    var forecatTimes = Ext.getDom("forecastTime").value;
    ...
    if (el.id == "p10") {
        ...
        if (Ext.getDom("dd1er").className == "radioChecked")// xuehui 12-06
...
//        el.className = "singleHazeLevel singleHazeLevel_Selected";
    if (el.className == "radioChecked") {
        if (id == "dd1er") {
            ...
            radioClickDateII('dd3er')
            radioClickDateIII('dd5er')
        }
        else {
           // $('#' + id).css("margin-top", "-5px");
            id = "dd1er";
            ...
            radioClickDateII('dd4er')
            radioClickDateIII('dd6er')
        }
    ...
    strPublishDate = strPublishDate  +" "+hour;
       ...
       Ext.Ajax.request({
           ...
           success: function (response) {
            ...
            function valign(o) {
                var a = Math.ceil(o.value.length / o.cols);
                o.style.paddingTop = ((o.rows - a) / 2) * parseInt(o.style.fontSize);
            }



            function DateChange(el) {
                $("#forecastTime").val(el.value);
                radioClickModule(oldPeriod);
            }
