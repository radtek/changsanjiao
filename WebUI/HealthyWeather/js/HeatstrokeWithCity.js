...
var heatstrokeValues = { '1': '不易中暑，应注意饮食清淡，睡眠充足。',  '2': '气温较高，可能导致中暑，请注意防暑降温，尽量减少午后或气温较高时长时间在露天环境中活动。', '3': '高温天气，较易发生中暑，请注意防暑降温，减少午后或气温较高时在日光下暴晒及在露天环境中活动。', '4': '高温炎热，容易发生中暑，请注意采取防暑降温措施，尽量避免午后或高温时段在日光下暴晒及在露天环境中活动。', '5': '极度酷热天气，极易发生中暑，请采取积极有效的防暑降温措施，避免在日光下暴晒，避免高温时段或高温环境中的户外活动。', '6': '极度酷热天气，极易发生中暑，请采取积极有效的防暑降温措施，避免在日光下暴晒，避免高温时段或高温环境中的户外活动。'};
...
function RefershMap(index) {
    $.each($(".firstPolText"), function (i, n) {
        ...
        if (index == 1 || index == 2) {
            ...
            var z = eval(heatstrokeValues);
            var cx = $($(".aqiValueII")[i]);
            cx.val(z[polLevelIndexs]); //2017-2-15 薛辉
        }
...
function refershChildForecast(flag){
    ...
    Ext.Ajax.request({
        ...
        params: { priod: priod, type: "中暑", forecastTime: forecastTime },
        success: function (response) {
...
function refershChild() {
    ...
    Ext.Ajax.request({
        ...
        params: { priod: priod, type: "中暑", forecastTime: forecastTime, user: user },
        success: function (response) {
            if (response.responseText != "") {
                var vs = response.responseText.split('*');
