...
function tableImage(el, entityName) {
    ...
    var methodName;
    if (entityName == "PM2_5" ||
         entityName == "HuadongPM25" ||
        entityName == "ChangPM25")
        methodName = "chinaTable";
    else if (entityName == "pm2.5_class") {
        methodName = "DateChinaTable";
        subString = Pm25Class;
    } else if (entityName == "ShanghaiRTPM25") {
        lastIndex = nameProp.lastIndexOf(".");
        subString = nameProp.substr(0, lastIndex);
        secIndex = subString.lastIndexOf("_");
        subString = subString.substr(secIndex + 1);
        methodName = "ShanghaiTable";
    }
    else if ( entityName == "ShanghaiRTPM10") {
        lastIndex = nameProp.lastIndexOf(".");
        subString = nameProp.substr(0, lastIndex);
        secIndex = subString.lastIndexOf("_");
        subString = subString.substr(secIndex + 1);
        methodName = "ShanghaiRTPM10Table";
    }
    else if ( entityName == "ShanghaiRTO3") {
        lastIndex = nameProp.lastIndexOf(".");
        subString = nameProp.substr(0, lastIndex);
        secIndex = subString.lastIndexOf("_");
        subString = subString.substr(secIndex + 1);
        methodName = "ShanghaiRTO3Table";
    }
    else {
    ...
    Ext.Ajax.request({
        ...
        success: function (response) {
            if (response.responseText != "") {
                ...
                if (methodName == "ShanghaiTable" || methodName == "ShanghaiRTO3Table" || methodName == "ShanghaiRTPM10Table") {
                    Ext.getDom("tableDiv").style.height = "420px";
                    Ext.getDom("tableDiv").style.width = "578px";
                } else {
                    Ext.getDom("tableDiv").style.height = "650px";
                    Ext.getDom("tableDiv").style.width = "550px";
                }
...
function quxianImage(entityName) {
    ...
    if (entityName == "PM10" ||
      entityName == "HuadongPM10" ||
      entityName == "ShanghaiRTPM10" ||
      entityName == "ChangPM10") {
        methodName = "chinaPM10Quxian";
        titleText = "PM10污染物小时曲线";
    }

    if (entityName == "O3" ||
      entityName == "ShanghaiRTO3" ||
      entityName == "HuadongO3") {
        methodName = "chinaPM10Quxian";
        titleText = "O3污染物小时曲线";
    }
