...
function trickQueryList(Datetime) {
    ...
    var mapType = thematic;
    Ext.Ajax.request({
        url: getUrl('MMShareBLL.DAL.Forecast', 'trickQueryListCSJ'),
        params: { Datetime: Datetime, entityName: id, json: json, period: period,mapType:mapType },
        success: function (response) {
            if (response.responseText != "") {
                var contentNone = Ext.getDom("contentNone");
                contentNone.innerHTML = response.responseText;
...
function showOne(entityName, type, src, time, type, Period) {
    var dateTime =time.replace(/-/g, "/"); //转换成Data();
   // var date = dateTime.format("Y-m-d");
    Ext.getDom("OnlyOne").innerHTML = "";
    //var hour = dateTime.getHours();
    var time = Ext.getDom("time").value ;
...
//  wb  2017.6.21
var thematic = "point";
function selThematicMap(id) {
    var idType = Ext.getDom(id);
    if (idType.className == "radioUnChecked") {
        idType.className = "radioChecked";
    }
    if (thematic != "") {
        var oldMap = Ext.getDom(thematic);
        oldMap.className = "radioUnChecked";
        thematic = id;
    }
    ReduceSelect(0);
}
