...
namespace MMShareBLL.DAL
{
    ...
    public class PublicLog
    {
        ...
        public string chinaQuxian(string startDateTime, string endDateTime, string initStation)
        {
            ...
            {
                strSQL = string.Format("SELECT  DATEDIFF(S,'1970-01-01 00:00:00',timepoint) AS timepoint,value from CHINA_RT_CITY_AVERAGE where Area='{0}' {1} ORDER BY timepoint", chinaStationID[i], strWhere);
                str = str + strSQL + ";";
        ...
        public string DatechinaQuxian(string startDateTime, string endDateTime, string initStation)
        {
            ...
            for (int i = 0; i < chinaStationID.Length; i++)
            {
                strSQL = string.Format("SELECT  DATEDIFF(S,'1970-01-01 00:00:00',timepoint) AS timepoint,value from CHINA_RT_CITY_DATEAVERAGE where Area='{0}' {1} ORDER BY timepoint", chinaStationID[i], strWhere);
                str = str + strSQL + ";";
        ...
        public string normalTable(string dateTime, string flag)
        {
            ...
            {
                strSQL = string.Format("SELECT A.area, value, aqi FROM (select * from  CHINA_RT_CITY_AVERAGE WHERE (itemid = 1) AND (timepoint ='{0}'))as A left join China_RT_city_station on a.area=China_RT_city_station.area order by  China_RT_city_station.id ", dateTime);
                dt = m_DatabaseS.GetDataTable(strSQL);
            }
            else if (flag == "PM10Air")//实时监测PM10
            {
                strSQL = string.Format("SELECT A.area, value, aqi FROM (select * from  CHINA_RT_CITY_AVERAGE WHERE (itemid = 2) AND (timepoint ='{0}'))as A left join China_RT_city_station on a.area=China_RT_city_station.area order by  China_RT_city_station.id ", dateTime);
                dt = m_DatabaseS.GetDataTable(strSQL);
            }
            else
            {//实时监测PM2.5重污染区
                strSQL = string.Format("SELECT A.area, value, aqi FROM (select * from  CHINA_RT_CITY_DATEAVERAGE WHERE (itemid = 1) AND (timepoint ='{0}'))as A left join China_RT_city_station on a.area=China_RT_city_station.area order by  China_RT_city_station.id ", dateTime);
                dt = m_DatabaseS.GetDataTable(strSQL);
            }
            string returnStr = htmlJoson(dt, dateTime, flag);
            return returnStr;
