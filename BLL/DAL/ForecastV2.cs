...
namespace MMShareBLL.DAL
{
    ...
    public class ForecastV2
    {
        ...
        public DataTable GetLeftPanelTree(string id)
        {
            string strSQL = "SELECT T_ImageProduct2.ENTITYNAME,ALIGN,PERIOD,AliasName,T_ENTITY.HINT,CLASS,cssName,MenuName,flag  FROM T_ImageProduct2 LEFT OUTER JOIN T_ENTITY ON T_ENTITY.ENTITYNAME = T_ImageProduct2.ENTITYNAME WHERE MODULENAME = '" + id + "' ORDER BY ORDERID,CLASS,MenuName";
            return m_Database.GetDataTable(strSQL);
        }
        ...
        private string GetLastestDatetime(Entity entity,string type)
        {
            ...
            {
                if(type.IndexOf(",")>0)
                    strWhere = strWhere + " AND  Station in" + returnSQLStr(type); 
                else
                    strWhere = strWhere + " AND  Station='" + type + "'"; 
        ...
        public DataTable QueryDataTable(string Datetime, string entityName, string json,string period)
        {
            ...
            if (Datetime != "")
            {
                ...
                if (info[7] != "" && info[1] != "全国" && info[1] != "华东" && info[1] != "长三角")
                {                    
                    if (info[1] != "")
                    {
                        int periods = int.Parse(period);
                ...
                if (entity.Condition != "")
                    strWhere = strWhere + " AND " + entity.Condition;
                if (info[1] == "全国" || info[1] == "华东" || info[1] == "长三角")
                    strWhere = strWhere + " AND  station='" + info[7] +"' ";
        ...
        public DataTable QueryDataTableForMM(string Datetime, string entityName, string json, string period)
        {
            ...
            if (Datetime != "")
            {
                ...
                if (info[7] != "" && info[1] != "全国" && info[1] != "华东" || info[1] != "长三角")
                {
                    strTypeSQL = "ORDER BY  CHARINDEX(Station," + "'" + info[7] + "'" + ")";
                    if (info[1] != "")
                    {
                        int periods = int.Parse(period);
                        string forecastDate = DateTime.Parse(Datetime).AddHours(-periods).ToString("yyyy-MM-dd HH:00:00");
                        ...
                        strWhere = " WHERE ForecastDate='" + Datetime + "'";
                    strWhere = strWhere + "  AND Station in " + returnSQLStr(info[7]);
                }
                else
                    strWhere = " WHERE ForecastDate<='" + Datetime + "'";
                if (entity.Condition != "")
        ...
        public DataTable QueryTimeList(string entityName, string entityObj,string json)
        {
            ...
            if (!bl)
            {
                if (info[1] != "全国" && info[1] != "华东x" && info[1] != "长三角x")
                {
                    if (info[7] != "")
                    {
                        if (info[7].IndexOf(",") <= 0)
