...
namespace MMShareBLL.DAL
{
    public class ComForecastII
    {
        ...
        public string GetAQIChart(string fromDate, string toDate, string station)
        {
            ...
            string dateFiled = "  [Start] AS [END] ";

            string dateFileAdd = "   LST AS LST ";
            ...
            if (station == "CM")
            {
                ...
                str = "SELECT  LST AS [END] ,ROUND(Value*1000,1) as Value from Data_RT_Site WHERE LST BETWEEN '" + from + "' AND '" + to + "' AND  SiteID=249 AND AQIItemID=101 ORDER BY LST ";
            }
            if (station == "QP")
            {
                ...
                str = "SELECT  LST AS [END] ,ROUND(Value*1000,1) as Value from Data_RT_Site WHERE LST BETWEEN '" + from + "' AND '" + to + "' AND  SiteID=203 AND AQIItemID=101 ORDER BY LST ";
            }
            if (station == "NJ")
            {
                ...
                str = "SELECT TimePoint AS [END] ,PM2_5 from [semc_dmc].[dbo].[China_RT_CNEMC_Data] WHERE TimePoint BETWEEN '" + from + "' AND '" + to + "' AND  Area = '南京' AND PositionName ='草场门' ORDER BY TimePoint ";
            }
            if (station == "PD")
            {
