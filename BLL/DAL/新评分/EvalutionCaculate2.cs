...
{
    public class EvalutionCaculate2
    {
        ...
        public string UVDayScore(string dateTime)
        {
            string dtFrom = DateTime.Parse(dateTime).ToString("yyyy-MM-01 00:00:00");
            string dtTo = DateTime.Parse(dateTime).Date.AddMonths(1).AddDays(-1).ToString("yyyy-MM-dd 23:59:59");
            string strSQL = string.Format("SELECT LST,ForecastGrade16, ForecastGrade10, RTGrade, ForecastAccuracy16, ForecastAccuracy10, Score FROM T_UVEvaluate WHERE LST BETWEEN '{0}' and '{1}' ORDER by LST", dtFrom, dtTo);
            ...
            if (hazeTable.Rows.Count > 0)
            {
                foreach (DataRow rows in hazeTable.Rows)
                {
                    ...
                    sb.Append("<td class='tableRowUV' >" + rows["Score"] + "</td>");
                    sb.Append("</tr>");
                }
            }
            sb.Append("</table>");
        ...
        public string HazeDaysScore(string dateTime)
        {
            try
            {
                string dtFrom = DateTime.Parse(dateTime).ToString("yyyy-MM-01 00:00:00");
                string dtTo = DateTime.Parse(dateTime).Date.AddMonths(1).AddDays(-1).ToString("yyyy-MM-dd 23:59:59");
                string strSQL = string.Format("SELECT LST,Forecast05, Forecast17, ForecastGrade05, ForecastGrade17, RtDay, RtAllDay,Score FROM T_HazeEvaluate WHERE LST BETWEEN '{0}' and '{1}' ORDER by LST", dtFrom, dtTo);
                DataTable hazeTable = m_Database.GetDataTable(strSQL);
                StringBuilder sb = new StringBuilder();
                sb.Append("<table   width='100%' border='0' cellpadding='0' cellspacing='0'>");
                sb.Append("<tr>");
                ...
                sb.Append("<td class='tabletitleHazeDay'>实况05时</td>");
                sb.Append("<td class='tabletitleHazeDay'>实况17时</td>");
                sb.Append("<td class='tabletitleHazeDay'>日预报评分</td>");
                sb.Append("</tr>");
                if (hazeTable.Rows.Count > 0)
                {
                    foreach (DataRow rows in hazeTable.Rows)
                    {
                        sb.Append("<tr>");
                        sb.Append("<td class='hazeTableRow' >" + DateTime.Parse(rows[0].ToString()).ToString("yyyy月MM日dd日") + "</td>");
                        sb.Append("<td class='hazeTableRow' >" + rows["Forecast05"] + "</td>");
                        sb.Append("<td class='hazeTableRow' >" + rows["Forecast17"] + "</td>");
                        sb.Append("<td class='hazeTableRow' >" + rows["ForecastGrade05"] + "</td>");
                        sb.Append("<td class='tableRow' >" + rows["ForecastGrade17"] + "</td>");
                        if (Convert.IsDBNull(rows["RtDay"]) == false)
                        {
                            if (Convert.ToInt16(rows["RtDay"]) > 0)
                            {
                                sb.Append("<td class='hazeTableRow' >有霾</td>");
                            }
                            else
                            {
                                sb.Append("<td class='hazeTableRow' >无霾</td>");
                            }
                        }
                        else { sb.Append("<td class='hazeTableRow' ></td>"); }
                        if (Convert.IsDBNull(rows["RtAllDay"]) == false)
                        {
                            if (Convert.ToInt16(rows["RtAllDay"]) > 0)
                            {
                                sb.Append("<td class='hazeTableRow' >有霾</td>");
                            }
                            else
                        ...
                        sb.Append("<td class='hazeTableRow' >" + rows["Score"] + "</td>");
                        sb.Append("</tr>");
                    }
                }
                //else
                ...
                //    strSQL = string.Format("SELECT   LST, convert(varchar(2),[ReTime],114) as ReTime, (Haze-1), UserName FROM T_24Haze WHERE LST between '{0}' and '{1}' order by lst", dtFrom, dtTo);
                ...
                //            {
                //                sm17.Append("<td class='hazeTableRow' >" + filterRow[0]["Haze"] + "</td>");
                //                sm05.Append("<td class='hazeTableRow' >" + returnHazeStyle(filterRow[0]["Haze"].ToString()) + "</td>");
                //            }
                //            else
                //            {
                //                sm17.Append("<td class='hazeTableRow' >-</td>");
                //                sm05.Append("<td class='hazeTableRow' >-</td>");
                //            }
                ...
                //        if (filterRow.Length > 0)
                //        {
                //            if (Convert.IsDBNull(filterRow[0]["NoneDays05"]) == false)
                //            {
                //                if (Convert.ToInt16(filterRow[0]["NoneDays05"]) > 1)
                //                {
                //                    sb.Append("<td class='hazeTableRow' >有霾</td>");
                //                }
                //                else
                ...
                //            }
                //            else { sb.Append("<td class='hazeTableRow' ></td>"); }
                //            if (Convert.IsDBNull(filterRow[0]["NoneDays17"]) == false)
                //            {
                //                if (Convert.ToInt16(filterRow[0]["NoneDays17"]) > 1)
                //                {
                //                    sb.Append("<td class='hazeTableRow' >有霾</td>");
                //                }
                //                else
                ...
                //        else
                //        {
                //            sb.Append("<td class='hazeTableRow' >-</td>");
                //            sb.Append("<td class='hazeTableRow' >-</td>");
                //        }
                //        //for (int k = 0; k < 6; k++)
                //        for (int k = 0; k < 1; k++)
        ...
        public string HazeEvaluate(string TimeDate)
        {
            ...
            DataSetForcast.T_HazeEvaluateDataTable dt_hazeEvalution = new DataSetForcast.T_HazeEvaluateDataTable();

            DateTime beginTime = DateTime.Parse(DateTime.Parse(TimeDate).ToString("yyyy-MM-dd"));
            DateTime endTime = DateTime.Parse(DateTime.Parse(TimeDate).ToString("yyyy-MM-31"));
            try
            {
                for (DateTime startTime = beginTime; startTime <= endTime; startTime = startTime.AddDays(1))
                {

                    //计算每天的找到对应的数据源   预报数据  + 实况数据

                    //预报数据
                    string forecast_sql = "select LST,ReTime,Haze-1 as Haze,UserName from T_24Haze where convert(varchar(10),ReTime,120)='{0}'";
                    forecast_sql = string.Format(forecast_sql, startTime.ToString("yyyy-MM-dd"));
                    DataTable dt_forecast = m_Database.GetDataTable(forecast_sql);

                    if (dt_forecast == null || dt_forecast.Rows.Count <= 0 || dt_forecast.Rows.Count < 2)
                        continue;

                    //05时和17时的预报Lst找出来
                    DataRow rows_forecast05 = null;
                    DataRow rows_forecast17 = null;
                    if (dt_forecast.Rows[0]["ReTime"].ToString().IndexOf("05:") >= 0)
                    {
                        rows_forecast05 = dt_forecast.Rows[0];
                        rows_forecast17 = dt_forecast.Rows[1];
                    }
                    else
                    {
                        rows_forecast17 = dt_forecast.Rows[0];
                        rows_forecast05 = dt_forecast.Rows[1];
                    }

                    //实况数据找出 05  17  
                    string rt_sql = "select * from T_RtData where lst='{0}'";
                    string rt_sql05 = string.Format(rt_sql, rows_forecast05["LST"]);
                    DataTable dt_rt05 = m_Database.GetDataTable(rt_sql05);  //  05
                    string rt_sql17 = string.Format(rt_sql, rows_forecast17["LST"]);
                    DataTable dt_rt17 = m_Database.GetDataTable(rt_sql17);  //  17

                    if (dt_rt05 == null || dt_rt05.Rows.Count <= 0)
                        continue;

                    if (dt_rt17 == null || dt_rt17.Rows.Count <= 0)
                        continue;

                    int rtHaze05 = 0;
                    int rtHaze17 = 0;

                    if (Convert.ToInt16(dt_rt05.Rows[0]["NoneDays05"]) >= 2 || 
                        Convert.ToInt16(dt_rt05.Rows[0]["NoneDays17"]) >= 2)
                    { rtHaze05 = 1; }

                    if (Convert.ToInt16(dt_rt17.Rows[0]["NoneDays05"]) >= 2 || 
                        Convert.ToInt16(dt_rt17.Rows[0]["NoneDays17"]) >= 2)
                    { rtHaze17 = 1; }

                    //算法
                    DataRow haze_row = dt_hazeEvalution.NewRow();
                    haze_row["LST"] = startTime;
                    haze_row["UserID"] = rows_forecast17["UserName"];
                    haze_row["Forecast05"] = returnHazeStyle(rows_forecast05["Haze"].ToString());
                    haze_row["Forecast17"] = returnHazeStyle(rows_forecast17["Haze"].ToString());
                    haze_row["ForecastGrade05"] = rows_forecast05["Haze"];
                    haze_row["ForecastGrade17"] = rows_forecast17["Haze"];
                    haze_row["RtAllDay"] = rtHaze17;
                    haze_row["RtDay"] = rtHaze05;
                    haze_row["Score"] = returnHazeScore(Convert.ToInt16(rows_forecast05["Haze"]), 
                                                        Convert.ToInt16(rows_forecast17["Haze"]),
                                                        rtHaze05, rtHaze17);

                    dt_hazeEvalution.Rows.Add(haze_row);//最后每天的结果

                }

                //插入数据库
                string strSQL = string.Format("DELETE T_HazeEvaluate WHERE LST between  '{0}' AND '{1}'", beginTime, endTime);
                m_Database.Execute(strSQL);
                bool flag = m_Database.BulkCopy(dt_hazeEvalution);
                if (flag)
                    return "评分成功";
                else 
                    return "评分失败";
            }
            catch { return "评分失败,请检查有没有导入实况数据！"; }
        }

        /// <summary>
        /// 霾实况数据入库 2018.3.7 by 孙明宇
        /// </summary>
        /// <param name="setDate"></param>
        public void RTtoDatabase(string setDate)
        {
            try
            {
                string path = System.Web.HttpContext.Current.Server.MapPath("~/") + "CJDATA\\Excel\\" + setDate;
                ...
                if (Directory.Exists(path))
                {
                    ...
                    {
                        var dataTxt = File.Open(listfile[i].FullName, FileMode.Open);

                        using (var stream = new StreamReader(dataTxt, System.Text.Encoding.GetEncoding("gb2312")))
                        {
                        ...
                        dataTxt.Close();
                        GC.Collect();
                        for (int j = 1; j < txt.Count; j++)
                        {
                            string[] line = Regex.Split(txt[j], "\\s+", RegexOptions.IgnoreCase);
                            db.Execute("select * from T_RTData where LST='" + Convert.ToDateTime(line[0]) + "'",
                                "update T_RTData set NoneDays05='" + line[1] + "', NoneDays17 = '" + line[2] + "' where LST='" + line[0] + "'",
                                "insert into T_RTData (LST,NoneDays05,NoneDays17) values('" + Convert.ToDateTime(line[0]) + "','" + line[1] + "','" + line[2] + "')");
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// 计算霾日评分 2018-03-08 by 孙明宇
        /// </summary>
        /// <param name="Forecast05"></param>
        /// <param name="Forecast17"></param>
        /// <param name="rtHaze05"></param>
        /// <param name="rtHaze17"></param>
        /// <returns></returns>
        private int returnHazeScore(int Forecast05, int Forecast17, int rtHaze05, int rtHaze17)
        {
            int Score = 0;
            if (rtHaze05 == 0 && Forecast05 == 0)
                 Score = 10;
            else
            {
                if (Forecast05 == 0) 
        ...
        public string PersonnalScoreData(string dateTime)
        {
            ...
            sb.Append("<td class='tabletitlePerson' >漏登记</td>");
