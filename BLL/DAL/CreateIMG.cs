...
namespace MMShareBLL.DAL
{
    class CreateIMG
    {
        Dictionary<string, string> m_AreaInfo; Dictionary<string, string> m_AreaData; DateTime m_Time; string m_Filename; string m_Basepath;
        public CreateIMG( Dictionary<string, string> AreaData, DateTime time, string filename, string basepath)
        {
       ...
       public string DealData()
        {
            try
            {
               ...
               // resultGraphics.TextRenderingHint = TextRenderingHint.AntiAlias;
                ...
                string retime = "";
                if (hh == 8)
                    title = "预报时间:" + yy.ToString() + "年" + mm.ToString().PadLeft(2, '0') + "月" + dd.ToString().PadLeft(2, '0') + "日";
                if (hh == 20)
                    title = "预报时间:" + m_Time.AddDays(1).ToString("yyyy") + "年" + m_Time.AddDays(1).ToString("MM") + "月" + m_Time.AddDays(1).ToString("dd") + "日";

                SetTextElement(title, 36, 80, 19, bitmapSource, "宋体", Color.Black);
                retime = "上海中心气象台" + yy.ToString() + "年" + mm.ToString().PadLeft(2, '0') + "月" + dd.ToString().PadLeft(2, '0') + "日" + hh.ToString().PadLeft(2, '0') + "时发布";
                SetTextElement(retime, 272, 1060, 20, bitmapSource, "宋体", Color.Black);
