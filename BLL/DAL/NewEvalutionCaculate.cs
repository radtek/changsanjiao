...
namespace MMShareBLL.DAL
{
    class NewEvalutionCaculate
    {
        ...
        public string ReturnAllData(string dateMonth)
        {
            ...
            {
                //雾霾预报
                strSQL = string.Format("select * from T_24Haze where  ReTime between '{0}' and '{1}'", beginTime, endTime);
                ...
                //紫外线预报+实况
                strSQL = string.Format("SELECT convert(varchar(10),DATEADD(day,0,[ReTime]),120)+' 00:00:00' as LST, convert(varchar(2),[ReTime], 114) as hour, UVAB,[Index], UserName  FROM T_TbUVS WHERE ReTime between '{0}' and '{1}' order by lst", beginTime, endTime);
                ...
                DataTable itemLive = JsonToDataTable(LiveStr);
                //国家局预报
                strSQL = string.Format("select * from T_ChinaValue where  LST between '{0}' and '{1}' AND Module ='Manual'", beginTime, endTime);
                DataTable CNFore = m_Database.GetDataTable(strSQL);
                //分时段预报
                strSQL = string.Format("select * from T_ForecastGroup where  durationID in (4,1,2,3) AND ForecastDate BETWEEN'{0}' AND '{1}' AND PERIOD =24 AND Module ='WRF'", beginTime, endTime);
                DataTable durFore = m_Database.GetDataTable(strSQL);
                //24小时预报
                strSQL = string.Format("SELECT * FROM T_ForecastGroup where durationID=7 AND ForecastDate BETWEEN'{0}' AND '{1}' AND PERIOD =24 AND Module ='WRF'", beginTime, endTime);
                ...
                sb.Append("<td class='tabletitlePerson2' colspan='4'>霾预报</td>");
                sb.Append("<td class='tabletitlePerson2' colspan='4'>紫外线预报</td>");
                sb.Append("<td class='tabletitlePerson2' colspan='7'>环保局</td>");
                sb.Append("<td class='tabletitlePerson2' colspan='5'>24小时预报</td>");
                sb.Append("<td class='tabletitlePerson2' colspan='5'>24小时IAQI</td>");
                sb.Append("<td class='tabletitlePerson2' colspan='18'>分时段浓度（个人）</td>");
                ...
                sb.Append("<td class='tabletitlePersonOther' rowspan='2'>紫外线实况</td>");
                sb.Append("<td class='tabletitlePersonOther' rowspan='2'>16时预报</td>");
                sb.Append("<td class='tabletitlePersonOther1' rowspan='2'>10时预报</td>");
                ...
                sb.Append("<td class='tabletitlePersonOther' rowspan='2'>CO</td>");
                sb.Append("<td class='tabletitlePersonOther' rowspan='2'>PM10</td>");
                sb.Append("<td class='tabletitlePersonOther' rowspan='2'>SO2</td>");
                ...
                sb.Append("<td class='tabletitlePersonOther1' colspan='4'>上半夜(今日)</td>");
                sb.Append("<td class='tabletitlePersonOther1' colspan='4'>下半夜</td>");
