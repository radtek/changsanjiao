...
namespace MMShareBLL.DAL
{
    ...
    public class Forecast
    {
        ...
        public string QueryListHtml(string entityName, string entityObj)
        {
            ...
            else if (entityName==""||entityName== "SeaLevelPressure") {
                strSQL = "SELECT ('Product/' + folder + '/' + name) AS DM, (CASE WHEN Period IS NULL THEN CONVERT(varchar(16),ForecastDate, 120) ELSE SUBSTRING(CONVERT(varchar(16),ForecastDate, 120), 0, 5)+'年'+SUBSTRING(CONVERT(varchar(16), ForecastDate, 120), 6, 2) + '月' + SUBSTRING(CONVERT(varchar(16), ForecastDate, 120), 9, 2) + '日' + SUBSTRING(CONVERT(varchar(16), ForecastDate, 120), 12, 2) + '时' END) AS MC,TYPE,Period FROM  " + entity.TableName + strWhere + "ORDER BY ForecastDate";
            }
            try
            {
        ...
        //wb  2017.6.21   
        //点击选择样式进行选择图片与QueryImgUrl函数相似，多了一个参数mapType——表示点击选择样式后的id
        public string QueryImgUrlII(DataTable dTable, string entityName, string[] info,string mapType,string period)
        {
            ...
            #region   对dTable表进行处理，如果maptype=point,则删除表中图片地址中包含K的行
            for (int i = 0; i < dTable.Rows.Count; i++)
            {
                //有几张区域分布图片的名称名称包含了K，所以单独处理
        ...
        public string GetCount() {
            ...
            string strSQL = "SELECT Count(*) FROM T_CancelRequest C  "+
                            "  INNER JOIN T_PubUser P ON P.UserID=C.UserID WHERE  (C.UserID IN (select UserID FROM T_PubUser " +
                             " WHERE  1=1  and   1=1 ) and isNull(C.PROCESSRESULT,'')<>'已处理') ";
            DataTable  dt = m_Database.GetDataTable(strSQL);
