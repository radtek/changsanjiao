...
namespace MMShareBLL.DAL
{
    public class CitysForecast
    {
        ...
        /// <returns></returns>
        public string GetAQIChart(string fromDate, string toDate,string city,string duration)
        {
            string strSQL = "";
            string strReturn = "";
            string x = "";
                ...
                DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd 00:00:00"))
            {
                strSQL = " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,lst from T_ForecastCity where Site='" + city + "' and " +
                         ...
                         " and Module='CUACE' and durationID='" + duration + "' and ITEMID='{0}'  ORDER BY LST asc ;";

                strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,LST from       where Site='" + city + "' and " +
                           " (ForecastDate = '" + DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd 20:00:00") + "' ) " +
                          " and Module='WRFChem' and durationID='" + duration + "' and ITEMID='{0}'  ORDER BY LST asc ;";

                strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,LST from T_ForecastCity where Site='" + city + "' and " +
                           " (ForecastDate = '" + DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd 20:00:00") + "' ) " +
                          " and Module='CMAQ' and durationID='" + duration + "' and ITEMID={0}  ORDER BY LST asc ;";
            ...
            else
            {
                ...
                strSQL = " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,lst from T_ForecastCity where Site='" + city + "' and " +
                          ...
                          " and Module='CUACE' and durationID='" + duration + "' and ITEMID='{0}' and PERIOD=48 ORDER BY LST asc ;";

                strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,LST from T_ForecastCity where Site='" + city + "' and " +
                          " (lst between '" + from + "' and '" + to + "' ) " +
                          " and Module='WRFChem' and durationID='" + duration + "' and ITEMID='{0}' and PERIOD=48 ORDER BY LST asc ;";

                strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END],value,LST from T_ForecastCity where Site='" + city + "' and " +
                          " (lst between '" + from + "' and '" + to + "' ) " +
                          " and Module='CMAQ' and durationID='" + duration + "' and ITEMID={0} and PERIOD=48 ORDER BY LST asc ;";
        ...
        public string GetAQIChartDB(string fromDate, string toDate, string city, string duration)
        {
            string strSQL = "";
            string strReturn = "";
            string x = "";
           ...
           // string siteID = "";
            ...
            if (DateTime.Parse(fromDate).ToString("yyyy-MM-dd 00:00:00") ==
                DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd 00:00:00"))
            {
                strSQL = " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,lst,ItemID from T_ForecastCity where Site='" + city + "' and " +
                         " (ForecastDate = '" + DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd 20:00:00") + "' ) " +
                         " and Module='CUACE' and durationID='"+duration+"' and ITEMID in ({0})  ORDER BY LST asc ;";

                strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,LST,ItemID from   T_ForecastCity    where Site='" + city + "' and " +
                          " (ForecastDate = '" + DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd 20:00:00") + "' ) " +
                         " and Module='WRFChem' and durationID='" + duration + "' and ITEMID in ({0})  ORDER BY LST asc ;";

                strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,LST,ItemID from T_ForecastCity where Site='" + city + "' and " +
                       " (ForecastDate = '" + DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd 20:00:00") + "' ) " +
                      " and Module='CMAQ' and durationID='" + duration + "' and ITEMID in ({0})  ORDER BY LST asc ;";

                strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,LST,ItemID from T_ForecastCity where Site='" + city + "' and " +
                           " (ForecastDate = '" + DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd 20:00:00") + "' ) " +
                          " and Module='WRFCMAQ' and durationID='"+duration+"' and ITEMID in ({0})  ORDER BY LST asc ;";

                strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,LST,ItemID from T_ForecastCity where Site='" + city + "' and " +
                          " (ForecastDate = '" + DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd 20:00:00") + "' ) " +
                         " and Module='CUACE9' and durationID='" + duration + "' and ITEMID in ({0})  ORDER BY LST asc ;";//增加cuace 9km  

                //加一个多模式集合黑线 2016-12-22
                strSQL += "select DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], avg(value),LST,ItemID from ( SELECT  avg(value) as 'value',LST,ITEMID,module from T_ForecastCity where Site='" + city + "' and " +
                          "   (ForecastDate = '" + DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd 20:00:00") + "' ) " +
                         "     and Module in ('CUACE9','WRFCMAQ','CMAQ','WRFChem','CUACE') " +
                           "    and durationID='" + duration + "' and ITEMID in ({0}) group by Module,LST,ITEMID ) x group by LST,ITEMID" +
                           "	ORDER  by lst asc ";
            ...
            else
            {
                ...
                strSQL = " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,lst,ItemID from T_ForecastCity where Site='" + city + "' and " +
                          " (lst between '" + from + "' and '" + to + "' ) " +
                          " and Module='CUACE' and durationID='" + duration + "' and ITEMID in ({0}) " + where + " ORDER BY LST asc ;";

                strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,lst,ItemID from T_ForecastCity where Site='" + city + "' and " +
                         " (lst between '" + from + "' and '" + to + "' ) " +
                         " and Module='WRFChem' and durationID='"+duration+"' and ITEMID in ({0}) "+where+" ORDER BY LST asc ;";

                strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,lst,ItemID from T_ForecastCity where Site='" + city + "' and " +
                         " (lst between '" + from + "' and '" + to + "' ) " +
                         " and Module='CMAQ' and durationID='"+duration+"' and ITEMID in ({0})  "+where+" ORDER BY LST asc ;";

                strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,LST,ItemID from T_ForecastCity where Site='" + city + "' and " +
                          " (lst between '" + from + "' and '" + to + "' ) " +
                          " and Module='WRFCMAQ' and durationID='"+duration+"' and ITEMID in ({0}) "+where+" ORDER BY LST asc ;";

                strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,LST,ItemID from T_ForecastCity where Site='" + city + "' and " +
                          " (lst between '" + from + "' and '" + to + "' ) " +
                          " and Module='CUACE9' and durationID='" + duration + "' and ITEMID in ({0}) " + where + " ORDER BY LST asc ;";//增加cuace 9km

                //加一个多模式集合黑线 2016-12-22
                strSQL += "select DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], avg(value),LST,ItemID from ( SELECT  avg(value) as 'value',LST,ITEMID,module from T_ForecastCity where Site='" + city + "' and " +
                          "   (lst between '" + from + "' and '" + to + "' ) " +
                         "     and Module in ('CUACE9','WRFCMAQ','CMAQ','WRFChem','CUACE') " +
                           "    and durationID='" + duration + "' and ITEMID in ({0}) " + where + " group by Module,LST,ITEMID ) x group by LST,ITEMID" +
                           "	ORDER  by lst asc ";
            }
            if (duration == "10")
            {
            ...
            {
                strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', DateAdd(dd,-1,time_point)) AS [END], pm25*1000 as 'pm2_5',DateAdd(dd,-1,time_point),{2}*1000 as 'pm10',Ozone1*1000 as 'o3',{4}*1000 as 'no2',{5}*1000 as 'CO',{6}*1000 as 'SO2' from T_24hAQI where area='" + city + "' and " +
                     " (DateAdd(dd,-1,time_point) between '" + from + "' and '" + to + "' ) and " +
            ...
            try
            {
                for (int i = 0; i < ItemArray.Length; i++)
                {
                    for (int index = 0; index < dst.Tables.Count; index++)
                    {
                        ...
                        for (DateTime dt = bt; dt <= endt; dt = GetDateTime(duration, dt))
                        {
                            bool bl = false;
        ...
        public string GetAQIChartDBPM()
        {
            ...
            string from = DateTime.Now.AddDays(-3).ToString("yyyy-MM-dd HH:00:00");
            string to = DateTime.Now.ToString("yyyy-MM-dd HH:00:00");



            strSQL = " SELECT DATEDIFF(S,'1970-01-01 00:00:00', [datetime]) AS [END], PM2_5 as 'value',[datetime],'1' as 'ItemID' from tbDust_1h where StationID='58367' and " +
                         " ([datetime] between  '"+from+"' and '"+to+"'   ) " +
                         "  ORDER BY [datetime] asc ;";

            strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', [datetime]) AS [END], PM2_5 as 'value',[datetime],'1' as 'ItemID' from tbDust_1h where StationID='99899' and " +
                  " ([datetime] between  '" + from + "' and '" + to + "'   ) " +
                  "  ORDER BY [datetime] asc ;";

            strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', [datetime]) AS [END], PM2_5 as 'value',[datetime],'1' as 'ItemID' from tbDust_1h where StationID='58363' and " +
                             " ([datetime] between  '" + from + "' and '" + to + "'   ) " +
                             "  ORDER BY [datetime] asc ;";

            strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', [time_point]) AS [END], PM2_5 as 'value',[time_point],'1' as 'ItemID' from T_CityAQI where area='上海市' and " +
                    " ([time_point] between  '" + from + "' and '" + to + "'   ) " +
                    "  ORDER BY [time_point] asc ;";

     

            string[] ItemArray = { "pm2_5"};
            string[] ItemIDS = { "1" };

            //先全部查询出来
            DataSet dst = m_Database.GetDataset(strSQL);
            ...
            try
            {
                for (int i = 0; i < ItemArray.Length; i++)
                {
                    for (int index = 0; index < dst.Tables.Count; index++)
                    {
                        DataTable dtElementNew = dst.Tables[index];
                        ...
                        DateTime bt = DateTime.Parse(dtElementNew.Rows[0][2].ToString());
                        DateTime endt = DateTime.Parse(dtElementNew.Rows[dtElementNew.Rows.Count - 1][2].ToString());
                        for (DateTime dt = bt; dt <= endt; dt = GetDateTime("", dt))
                        {
                            bool bl = false;
                            foreach (DataRow dr in dtElementNew.Rows)
                            {
        ...
        /// <param name="duration"></param>
        /// <returns></returns>
        public string GetAQIChartDBPJPC(string forecastDate, string ybDate)
        {
            try{
                forecastDate = DateTime.Parse(forecastDate).ToString("yyyy-MM-dd 20:00:00");
                ybDate = DateTime.Parse(ybDate).ToString("yyyy-MM-dd HH:mm:00");
                string modules = "WRFChem,CMAQ,WRFCMAQ,CUACE,CUACE9,多模式最优集成";
                string sites = "'上海市','济南市','青岛市','烟台市','德州市','聊城市','滨州市','荷泽市','合肥市','芜湖市','蚌埠市','淮南市','淮北市','铜陵市','安庆市','黄山市','马鞍山市','南京市','无锡市','徐州市','常州市','苏州市','南通市','扬州市','镇江市','连云港市','福州市','厦门市','莆田市','三明市','泉州市','杭州市','宁波市','温州市','嘉兴市','绍兴市','金华市','衢州市','台州市','丽水市','南昌市','九江市','鹰潭市','上饶市','景德镇市'";
                string sqls = "  SELECT  Site,Module,(t2.pm2_5-t1.value) as 'pc',t2.time_point from T_ForecastCity t1 inner join T_CityAQI t2 on t1.LST=t2.time_point " +
                             " and t1.[Site]=t2.area " +
                                             "  where t1.Site in (" + sites + ") and " +
                                              "  (t1.ForecastDate = '" + forecastDate + "' and t1. LST='" + ybDate + "') " +
                                              "  and t1.Module='WRFChem' and t1.durationID='10' and t1.ITEMID=1  " +
                             " UNION " +
                             " SELECT  Site,Module,(t2.pm2_5-t1.value) as 'pc',t2.time_point from T_ForecastCity t1 inner join T_CityAQI t2 on t1.LST=t2.time_point " +
                             "  and t1.[Site]=t2.area" +
                                          "     where t1.Site in (" + sites + ") and " +
                                           "  (t1.ForecastDate = '" + forecastDate + "' and t1. LST='" + ybDate + "') " +
                                           "     and t1.Module='CUACE' and t1.durationID='10' and t1.ITEMID=1  " +
                            " UNION" +
                            "  SELECT  Site,Module,(t2.pm2_5-t1.value) as 'pc',t2.time_point from T_ForecastCity t1 inner join T_CityAQI t2 on t1.LST=t2.time_point " +
                             "  and t1.[Site]=t2.area" +
                                              " where t1.Site in (" + sites + ") and " +
                                            "  (t1.ForecastDate = '" + forecastDate + "' and t1. LST='" + ybDate + "') " +
                                              "  and t1.Module='CMAQ' and t1.durationID='10' and t1.ITEMID=1  " +
                            " UNION" +
                             " SELECT  Site,Module,(t2.pm2_5-t1.value) as 'pc',t2.time_point from T_ForecastCity t1 inner join T_CityAQI t2 on t1.LST=t2.time_point " +
                             "  and t1.[Site]=t2.area" +
                                             "  where t1.Site in (" + sites + ") and " +
                                              "  (t1.ForecastDate = '" + forecastDate + "' and t1. LST='" + ybDate + "') " +
                                             "   and t1.Module='WRFCMAQ' and t1.durationID='10' and t1.ITEMID=1  " +
                            " UNION" +
                            " SELECT  Site,Module,(t2.pm2_5-t1.value) as 'pc',t2.time_point from T_ForecastCity t1 inner join T_CityAQI t2 on t1.LST=t2.time_point " +
                             "  and t1.[Site]=t2.area" +
                                              " where t1.Site in (" + sites + ") and " +
                                           "  (t1.ForecastDate = '" + forecastDate + "' and t1. LST='" + ybDate + "') " +
                                              "  and t1.Module='CUACE9' and t1.durationID='10' and t1.ITEMID=1  " +
                            " UNION" +
                             " select Site as 'Site','多模式最优集成' as 'Module',(avg(value)-avg(sk)) as 'pc',LST as 'time_point' from ( SELECT  site as 'Site',lst,itemID,avg(value) as 'value',module,max(t2.pm2_5) as 'sk' from T_ForecastCity " +
                                           "  t1 inner join T_CityAQI t2 on t1.LST=t2.time_point " +
                                            " and t1.[Site]=t2.area " +
                                            "  where Site in (" + sites + ") and " +
                                                 "  (t1.ForecastDate = '" + forecastDate + "' and t1. LST='" + ybDate + "') " +
                                                    "   and Module in ('CUACE9','WRFCMAQ','CMAQ','WRFChem','CUACE') " +
                                                      "  and durationID=10 and ITEMID=1 group by site,Module,LST,ITEMID ) x group by site,LST,ITEMID";
                string values = "";
                DataTable dt=   m_Database.GetDataTable(sqls);
                if (dt != null && dt.Rows.Count > 0) {
                    string[] sitess = sites.Split(',');
                    int index1 = 1;
                    foreach (string si in sitess) {
                        DataRow[] rows = dt.Select(" Site='"+si.Replace("'","")+"'");
                        if (rows != null && rows.Length > 0) { 
                             //找六个模型
                            int index2 = 1;
                            foreach(string mol in modules.Split(',')){
                                string v = ((index1 + "" + index2 + ":-#"));
                                foreach (DataRow row in rows) {
                                    if (mol == row["Module"].ToString()) {
                                        v=( (index1 + "" + index2 + ":" + Math.Round(double.Parse(row["pc"].ToString()),2)+"#"));
                                        break;
                                    }
                                }
                                values += v;
        ...
        /// <param name="duration"></param>
        /// <returns></returns>
        public string GetAQIChartDBII(string fromDate, string toDate, string city, string duration)
        {
            string strSQL = "";
            string strReturn = "";
            string x = "";
           ...
           // {
                strSQL = " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,lst,ItemID from T_ForecastCity where Site='" + city + "' and " +
                         " (ForecastDate = '" + DateTime.Parse(from).ToString("yyyy-MM-dd 20:00:00") + "' ) " +
                         " and Module='CUACE' and durationID='" + duration + "' and ITEMID in ({0})  ORDER BY LST asc ;";
        ...
        private string Disctinct(string text) {
        ...
        public string GetAQIChartDBIII(string fromDate, string toDate, string city, string duration,string model)
        {
            string strSQL = "";
            string strReturn = "";
            string x = "";
                ...
                DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd 00:00:00"))
            {
                ...
                foreach (string cityName in city.Split(','))
                {
                    string cityNames = cityName.Replace("'", "");
                    if (model == "CUACE15km")
                    {
                        strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,lst,ItemID,'" + cityNames + "' as '1' from T_ForecastCity where Site='" + cityNames + "' and " +
                                 " (ForecastDate = '" + DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd 20:00:00") + "' ) " +
                                 " and Module='CUACE' and durationID='" + duration + "' and ITEMID in ({0})  ORDER BY LST asc ;";
                    }
                    else if (model == "WRF-Chem")
                    {
                        strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,LST,ItemID,'" + cityNames + "' as '1' from   T_ForecastCity    where Site='" + cityNames + "' and " +
                                  " (ForecastDate = '" + DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd 20:00:00") + "' ) " +
                                 " and Module='WRFChem' and durationID='" + duration + "' and ITEMID in ({0})  ORDER BY LST asc ;";
                    ...
                    else if (model == "CMAQ10天")
                    {
                        strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,LST,ItemID,'" + cityNames + "' as '1' from T_ForecastCity where Site='" + cityNames + "' and " +
                                   " (ForecastDate = '" + DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd 20:00:00") + "' ) " +
                                  " and Module='WRFCMAQ' and durationID='" + duration + "' and ITEMID in ({0})  ORDER BY LST asc ;";
                    }
                    else if (model == "CUACE9km")
                    {
                        strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,LST,ItemID,'" + cityNames + "' as '1' from T_ForecastCity where Site='" + cityNames + "' and " +
                                   " (ForecastDate = '" + DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd 20:00:00") + "' ) " +
                                  " and Module='CUACE9' and durationID='" + duration + "' and ITEMID in ({0})  ORDER BY LST asc ;";
                    }
                    //cuace9km还没有解析
                }
            }
            else
            {
                ...
                foreach (string cityName in city.Split(','))
                {
                    ...
                    else if (model == "CUACE9km")
                    {
                        strSQL += " SELECT DATEDIFF(S,'1970-01-01 00:00:00', LST) AS [END], value,LST,ItemID,'" + cityNames + "' as '1' from T_ForecastCity where Site='" + cityNames + "' and " +
                                  " (lst between '" + from + "' and '" + to + "' ) " +
                                  " and Module='CUACE9' and durationID='" + duration + "' and ITEMID in ({0}) " + where + " ORDER BY LST asc ;";
                    }
                }
            }
        ...
        public DateTime GetDateTime(string duration, DateTime dt)
        {
            if (duration == "7")
                return dt.AddDays(1);
            else return dt.AddHours(1);
        }
