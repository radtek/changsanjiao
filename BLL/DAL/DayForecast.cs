   ...
   public  class DayForecast
    {
        ...
        public string TransStaionData(string fromDate)
        {
            ...
            string sql2 = "SELECT  SiteID, ParameterID, Max(CONVERT(VARCHAR(10),LST,111)) as LST,avg(Value) as Value " +
                       "FROM SEMC_DMS.DBO.Data WHERE siteID IN ( 255, 257, 254, 256, 258) AND parameterID IN " +
                           "( 92, 93, 94, 95, 106, 105, 76, 61, 62, 38) and CONVERT(VARCHAR(10),LST,111)='{0}' " +
                               "and DurationID=10  and (qccode in (0,1,2,3,4,5,6,10,11) ) and value>0 group by parameterID,siteID having(count(Lst)>=18) ";
