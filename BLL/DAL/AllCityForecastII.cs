...
{
    public class AllCityForecastII
    {
        private Database m_DatabaseMySQL;
        private Database m_Database;
        public AllCityForecastII()
        {
        ...
        public string GetForecast(string forecastDate)
        {
            ...
            forecastDate = DateTime.Parse(forecastDate).ToString("yyyy-MM-dd 00:00:00");
            //给预报数据赋值
            string strSQL = " select * from forecast_day_csj where forecastDate='" + forecastDate + "' and citycode in ('50745','50873','54453','54337','54324','54237','54497','54347','54094','50850','50853','54827','50953');";
            DataTable dt = m_Database.GetDataTable(strSQL);
            ...
            strSQL = " (select Site,LST,DATEDIFF(day,forecastDate,LST)-1 as type, ITEMID, Value, AQI from T_ForecastSite " +
                   ...
                   " Site in ('50745','50873','54453','54337','54324','54237','54497','54347','54094','50850','50853','54827','50953') " +
                   ...
                   "    and Site in ('50745','50873','54453','54337','54324','54237','54497','54347','54094','50850','50853','54827','50953') " +
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (dr["AQI"].ToString() != "0" && !string.IsNullOrEmpty(dr["AQI"].ToString()))
                {
                    ...
                    AQI = int.Parse(dr["AQI"].ToString());
                    aqiExt = new AQIExtention(AQI);
                    returnDestribute = ParseAQIForAirQualityII(aqiExt.AQI, aqiExt.FirstPItemNoByGrade, true);
                    desrtibute = returnDestribute.Split('，');
                    if (desrtibute[1].Trim() == "优")
        ...
        public string GetForecastHistory()
        {
            ...
            string[] siteID = { "50745", "50873", "54453", "54337", "54324", "54237", "54497", "54347", "54094", "50850", "50853", "54827", "50953" };//新添加三个城市  2016-10-30  
            forecastDate = DateTime.Parse(forecastDate).ToString("yyyy-MM-dd 00:00:00");
        ...
        public string PublishForecast(string postJson, string forecastDate)
        {
            ...
            //MySQL数据库的批量数据处理
            strSQL = "DELETE from forecast_day_csj WHERE  forecastDate='" + startDate.ToString("yyyy-MM-dd") + "'  and citycode in ('50745','50873','54453','54337','54324','54237','54497','54347','54094','50850','50853','54827','50953')";
        ...
        public string AQIDescribeII(string content)
        {
            try
            {
                ...
                if (maxAQI != 0)
                {
                    returnStr = changeAQIValueII(maxAQI, id.Replace("AQ", ""), itemID);
                }
                return returnStr;
            }
            catch (Exception ex)
        ...
        public string changeAQIValue(int value, string id, string itemID)
        {
            ...
            string returnDestribute = ParseAQIForAirQualityII(aqiExt.AQI, "", true);
        ...
        public string changeAQIValueII(int value, string id, string itemID)
        {
        ...
        private string ParseAQIForAirQualityII(int aqi, string firstParameter, bool showDescription)
        {
            string description = "首要污染物";
            if (showDescription == false)
                description = "";
            ...
            if (strGrade == "重度污染到严重污染")
                strGrade = "重度-严重";
            if (strGrade == "良到轻度污染")
                strGrade = "良-轻度"; // xuehui -07-12 陈镭需求去掉到
            if (strGrade == "优到良")
                strGrade = "优-良";
        ...
        public string SaveSHEdits(string postJson, string forecastDate)
        {
            ...
            try
            {
                InsertIntoStateTable("cityForecast", DateTime.Now.ToString("yyyy-MM-dd 17:00:00"), DateTime.Now.ToString("yyyy-MM-dd 19:00:00"), "3", "2");
        ...
        public string GetSHForecastHistory(string forecastDate)
        {
            string[] siteID = { "31000" };
            forecastDate = DateTime.Parse(forecastDate).AddDays(-1).ToString("yyyy-MM-dd 00:00:00");
            //给预报数据赋值
            string strSQL = " select * from forecast_day_sh where forecastDate='" + forecastDate + "' and citycode in ('31000');";
            DataTable dt = m_Database.GetDataTable(strSQL);
            ...
            foreach (DataRow dr in dt.Rows)
            {
                if (dr.ItemArray[2].ToString() != "1")
                {
                    //颜色等级一类的特殊处理
                    if (dr["AQI"].ToString() != "0" && !string.IsNullOrEmpty(dr["AQI"].ToString()))
                    {
                        sb.Append(string.Format("'H{0}{1}1':'{2}',", dr[0].ToString(), (Convert.ToInt32(dr[2]) - 1).ToString(), dr["PM25"].ToString()));
                        sb.Append(string.Format("'H{0}{1}2':'{2}',", dr[0].ToString(), (Convert.ToInt32(dr[2]) - 1).ToString(), dr["PM10"].ToString()));
                        sb.Append(string.Format("'H{0}{1}3':'{2}',", dr[0].ToString(), (Convert.ToInt32(dr[2]) - 1).ToString(), dr["NO2"].ToString()));
                        sb.Append(string.Format("'H{0}{1}5':'{2}',", dr[0].ToString(), (Convert.ToInt32(dr[2]) - 1).ToString(), dr["O3"].ToString()));
                        sb.Append(string.Format("'H{0}{1}6':'{2}',", dr[0].ToString(), (Convert.ToInt32(dr[2]) - 1).ToString(), dr["CO"].ToString()));
                        sb.Append(string.Format("'H{0}{1}7':'{2}',", dr[0].ToString(), (Convert.ToInt32(dr[2]) - 1).ToString(), dr["SO2"].ToString()));
                        sb.Append(string.Format("'XH{0}{1}primeplu':'{2}',", dr[0].ToString(), (Convert.ToInt32(dr[2]) - 1).ToString(), dr["primeplu"].ToString()));
                        sb.Append(string.Format("'XH{0}{1}AQI':'{2}',", dr[0].ToString(), (Convert.ToInt32(dr[2]) - 1).ToString(), dr["AQI"].ToString()));
                        AQI = int.Parse(dr["AQI"].ToString());
                        aqiExt = new AQIExtention(AQI);
                        returnDestribute = ParseAQIForAirQualityII(aqiExt.AQI, aqiExt.FirstPItemNoByGrade, true);
                        desrtibute = returnDestribute.Split('，');
                        if (desrtibute[1].Trim() == "优")
                            sb.Append(string.Format("'XH{0}{1}kqzl':'{2}',", dr[0].ToString(), (Convert.ToInt32(dr[2]) - 1).ToString(), desrtibute[0]));
                        else
                            sb.Append(string.Format("'XH{0}{1}kqzl':'{2}',", dr[0].ToString(), (Convert.ToInt32(dr[2]) - 1).ToString(), desrtibute[0]));
                        sb.Append(string.Format("'XH{0}{1}text':'{2}',", dr[0].ToString(), (Convert.ToInt32(dr[2]) - 1).ToString(), desrtibute[1]));
                        sb.Append(string.Format("'XH{0}{1}color':'{2}',", dr[0].ToString(), (Convert.ToInt32(dr[2]) - 1).ToString(), aqiExt.Color));
                        sb.Append(string.Format("'XH{0}{1}tqxs':'{2}',", dr[0].ToString(), (Convert.ToInt32(dr[2]) - 1).ToString(), dr["ELE_overproof"].ToString()));
        ...
        public string UpLoadFileToFTP(string WordInfo)
        {
            string info = WordInfo;
            string fileName = "上海市10天空气质量预报_" + info.Split('_')[1];//文件名
            string Date = DateTime.Now.Year.ToString();
            string ftpURL = "ftp://" + ConfigurationManager.AppSettings["Forecast10dFTP"].ToString() + Date + '/' + fileName;
            string sourceFileName = WordInfo;

            string ftpUser = ConfigurationManager.AppSettings["Forecast10dUser"].ToString(); ;
            string ftpPassword = ConfigurationManager.AppSettings["Forecast10dPassWord"].ToString(); ;
            FileInfo fileInf = new FileInfo(sourceFileName);
            System.Net.FtpWebRequest ftp = GetRequest(ftpURL, ftpUser, ftpPassword);
