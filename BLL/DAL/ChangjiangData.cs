...
namespace MMShareBLL.DAL
{
    public class ChangjiangData
    {
        ...
        /// <returns></returns>
        public string  GetChangjiangData(string type,string area,string beginTime,string endTime){
           string jsonString = "";
            try
            {
                string sqlStr = "select * from T_ChangjiangData where {0} order by uploadTime desc";
                string whereStr = "1=1";
                ...
                whereStr += "  and (uploadTime between '"+(beginTime+" 00:00:00")+"' and '"+(endTime+" 23:59:59")+"') ";
                DataTable dt = m_Database.GetDataTable(string.Format(sqlStr,whereStr));
                jsonString = DataTableToJsonNew("ChangjiangData", dt);
            }
            catch (Exception ex)
            {
                jsonString = ex.Message;
